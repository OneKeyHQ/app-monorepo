...
3. 之后 `ServiceDeFi.ts:478-501` 的 `LocalPendingTxConfirmed` 会立即 `_runDeFiForceRefresh` 并安排 40s/80s fallback；generic action 最终 Success 后，DeFi tab 的回调现在直接按 action 携带的 `accountId/networkId` 调用 `refreshAccountDeFiPositionsAfterAction`，再通知 History 更新。历史分类确认仍可触发同一刷新，但 ServiceDeFi 会按 account/network 合并在飞请求。
...
**建议的最小修复**：保留并为 generic pending/undefined guard 增加回归断言（它已存在，不是新的行为改动）；pending 只记录 order/状态，不发 positions force refresh。最终 Success 必须直接把 action 的 `accountId + networkId` 路由到 ServiceDeFi，不能依赖 History 容器间接触发；ServiceDeFi 继续按 account/network 去重 immediate 请求并保留 40s/80s indexer-lag fallback。Borrow `handleBorrowSuccess` 对 undefined 的旁支缺陷仍应单独修复/测试，但它不是这段 Morpho 录屏的根因。
