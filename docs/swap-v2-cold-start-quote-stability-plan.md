...
> 状态：**核心稳定性保护与 Stock 展示快照已在 `codex/swap-v2-stability` 实现；自动化已收口，Desktop dev 部分 real-surface 场景已验证，完整 Release Gates 仍按本文 Pending 项阻断**<br>
...
| 领域                    | 已实现状态                                                                                                                                      | 不可突破的安全边界                                                                                                                    |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| SSE Provider / Review   | 当前 request / intent 的首个 actionable 候选立即 pin display + executable，并可打开 picker；其他安全门闩通过后可在 `done` 前进入 Review         | retained 旧报价不可执行；SSE active 时 effective AUTO 的 early Review 必须等当前候选 recommendation，CUSTOM 不等；Frozen Review 不变  |
| 报价展示稳定性          | 同一 display intent 刷新时保留上一次 committed amount / rate / info，后台静默询价，不再退回骨架屏                                               | 新请求立即作废旧执行报价；保留展示不等于保留执行权限                                                                                  |
| Stock 首帧快照          | 物理存储按 account 分槽；token detail、balance、chart、amount、selection 各自按独立 region owner 恢复与静默替换                                 | 快照只服务展示；cached amount / balance 在 canonical live owner ready 前不可编辑，也不得参与 Max、百分比、quote、Review 或执行        |
...
1. `QUOTE-FIRST-ACTIONABLE-001`：首个当前请求 actionable 候选到达、SSE 尚未 `done` 时，picker 可点击且 display / executable pin 到该候选；effective AUTO recommendation 或 CUSTOM override 及其余安全门闩就绪后，Review 可在 `done` 前打开。
2. `QUOTE-PIN-STABILITY-001`：同 Provider 的后续 streaming update 不改变 amount、rate、fee、route 等经济字段；手选可显式换 Provider，terminal 自动更新最多一次，已冻结 Review 永不变化。
3. `QUOTE-ACTIONABILITY-001`：`errorMessage`、非有限 / 非正 `toAmount`、malformed 或违反 min/max 的候选不可选择；BUY / SELL 均以 `fromAmount` 校验 limits。
4. `STOCK-DISPLAY-RESTORE-001`：同账户再次进入 Stock，按 selection / amount / token-detail / balance / chart 的各自 owner 恢复；live 成功分区替换，失败保持 last-good 且不回骨架。
...
4. 已把 Provider 流式候选与 retained display 分离：相同 `displayIntentFingerprint` 刷新可先保留旧展示，但它不可执行；当前 request 的首个 actionable 候选立即 pin display + executable，后续同 Provider streaming 只允许 AUTO recommendation 等显式非经济 enrich，手选可显式换 Provider，terminal 最多再自动提交一次。SELL 的展示意图只绑定 from 主动输入，BUY 只绑定 to 主动输入；派生输出、AUTO 建议滑点和 approval block 更新不会触发重新骨架化，execution fingerprint 仍保留两侧金额、精确滑点和 block 的严格校验。
...
| -------------------------------------- | ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| 冷启动完整 pair                        | Auto-core PASS；历史 Desktop 基线        | 只从同一候选接受完整 pair；live pair 就绪后原子切换，禁止 cold/live 半对混拼                                                                  | composite `SwapColdStartSeedV2`、TTL / provenance 全量迁移和五平台样本未实现                   |
...
| 普通 SSE Quote Session V2              | Auto-core PASS                           | UI session identity、logical service per-surface registry、先 reserve 后 await、严格事件接收、精确取消、V1 接口保留                           | transport 自动重连、Provider 去重、quiet / hard settlement 未实现                              |
| 报价展示稳定性                         | Auto-core PASS；Desktop dev Partial PASS | retained / candidate / pinned executable 分层；首个当前 actionable 早提交；同 Provider 经济字段固定；手选显式切换；terminal 最多一次更新      | quiet / hard 策略、自动重连与其余真实 runtime 尚未完成                                         |
| 原始语义意图失效                       | Auto-core；历史 Desktop SELL 基线        | SELL、LIMIT BUY、token、账户、receiver、slippage 变化在 debounce 前立即取消旧请求；输出投影复核当前意图                                       | property-based 全排列、LIMIT BUY 和五平台真实快速输入未完成                                    |
...
- **Auto-core PASS**：代码路径存在，且第 0.1 节统一 Jest / 静态门禁已经通过。它不等同于五平台 Release PASS。
- **部分实现**：安全边界只覆盖明确列出的路径，未覆盖项继续按阻断发布处理。
- **目标，未实现**：用于下一阶段设计和验收，当前代码、测试与 PR 不得引用为已交付能力。
- **Historical Desktop sample**：仅记录第 16.4 节旧基线的具体 Desktop 环境和场景，不是最终代码证据；未列出的账户类型、模式、入口、平台和网络条件继续视为 Pending。
...
- 按目标平台区分 runtime topology：iOS / Android / Extension 的 UI 与 background 隔离，以及 Desktop / Web 单 JS runtime 下的逻辑 owner、存储所有权和初始化时序。
...
| R-10 | quote list 按 Provider 流式写入，当前最佳报价随列表重算                                                               | 主金额在不同 Provider 结果间来回跳                                                           | 首个当前 actionable 候选原子 pin；同 Provider streaming 固定经济字段；terminal 最多一次更新     |
...
iOS、Android 与 Extension 的 UI `main` / background `bg` 是隔离 JS runtime / context；Desktop 与 Web 的 App main/background 代码当前在同一 JS runtime / thread 内执行。`kit-bg` 表示逻辑服务 owner，不自动证明物理 bg heap 或序列化边界。

| 对象 / 行为                                            | 物理 Runtime                                                             | 资源 / 逻辑 owner                                                     | JS heap / 时序要求                                                                     |
| ------------------------------------------------------ | ------------------------------------------------------------------------ | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| Mobile 冷启动快照与 Stock display snapshot             | iOS / Android UI `main`                                                  | MMKV 为进程级原生资源；Swap UI 是 Stock display cache 唯一逻辑 writer | UI `main` 持有自己的 JS wrapper / 反序列化对象；不得假设 `bg` ready 或允许 `bg` 读缓存 |
| Extension Stock display snapshot                       | UI context                                                               | 当前 `coldStartCacheStorage` adapter 为 no-op                         | 不得宣称跨重启恢复；background service worker 不得补第二 writer                        |
| Web / Desktop hydration、Jotai、Stock display snapshot | 单 JS runtime / thread                                                   | IndexedDB / 平台存储；Swap UI 仍是逻辑 writer                         | 账户、storage、service readiness 仍可异步到达，但不存在天然双 heap                     |
| 当前 Jotai quote / display / Review                    | iOS / Android `main`；Extension UI；Desktop / Web 本地 runtime           | `kit` UI/Jotai                                                        | 每个 surface/store 独立 owner；只接受当前 request / region owner                       |
| SSE、Quote / Build / Status 服务                       | iOS / Android `bg`；Extension background；Desktop / Web 同一本地 runtime | `ServiceSwap` / BackgroundApi 是逻辑 owner                            | split-runtime 平台以 envelope 序列化；Desktop / Web 不得虚构复制                       |
| Stock display cache 与 Swap execution service          | 前者 UI owner；后者按上行 service placement                              | 两者禁止共享执行真相                                                  | `ServiceSwap` 不读取 cached amount / balance；只认 live account/token/quote inputs     |
| Review、Build 参数冻结                                 | UI 创建；split-runtime 时 service 消费序列化副本；Desktop / Web 本地调用 | frozen review / logical build service                                 | 当前 UI 检查 reviewRevision；目标 service 复核 fingerprint / expiry                    |

split-runtime 平台的 main / bg JS bundles随版本锁定；实际要防的是 native 与 JS 协议版本不兼容，而不是假设 main 与 bg 会部署不同业务版本。V2 envelope 必须带 `protocolVersion`，未知版本失败关闭并回退到受控 V1 路径。Desktop / Web 结论必须标为单 runtime；只有具体 worker、Electron process 或 bridge 证据才能改写。
...
flowchart TD
  ...
  CQ --> PQ["First Current Actionable\nPin Display + Executable"]
  PQ --> UI
  PQ --> F["FrozenReviewSnapshot\n其余安全门闩通过"]
  CQ --> T["Terminal Final Commit\n每 request 最多一次"]
  T --> UI
...
| main quote commit       | `idle → requesting → settled`；失败为 `error`                                | request start 清旧 executable，可保留同 display intent 的旧 display；首个当前 actionable candidate 在 `requesting` pin display + executable；后续同 Provider 只 enrich 非经济字段；手选可换 pin；terminal 最多一次 final commit                                    |
...
#### 6.0.3 实际 first-actionable pin 与 bounded terminal commit

- Provider message 先进入当前 event 候选区；第一个通过 current request / fingerprint 与 actionability 校验的候选立即 pin `displayQuote + executableQuote`，Provider picker 可用，其他 Review 门闩通过后无需等 `done`。
- actionable 的统一定义是：没有 `errorMessage`，`toAmount` 有限且大于 0，min / max 可解析且均使用 `fromAmount` 校验；BUY / SELL 不分叉。Provider 列表的分类与 disabled 状态必须共用同一 predicate。
- SSE active 时，effective AUTO 模式若当前 SWAP 候选尚无 `autoSuggestedSlippage`，picker 仍可用但 early Review fail-closed；effective CUSTOM override 不等待 AUTO recommendation。该判断读取当前 session `slippageItem.key`，不能被 persisted global mode 误锁。
- 同一 Provider 的后续 streaming event 只允许合并显式安全的非经济 enrich（当前为 AUTO recommendation）；amount、rate、fee、route 等 pinned 经济字段不变。
- 手选 Provider 以 Provider identity 表达选择意图；当前 event 的该 Provider 首个 actionable 结果可显式换 pin。手选对象不依赖 JS object identity。
- `done` 或当前 event 权威完成最多再执行一次 final commit；它可更新未冻结的页面主金额，但不得修改已打开的 Frozen Review。
- Early Review 在 SSE active 时冻结当前 pin，但 `supportPreBuild=false`；不得让预构建交易消费仍在流中的可变候选。
...
- 当前没有 quiet window、hard deadline、自动重连与跨重连 Provider 去重；first-actionable pin + bounded terminal commit 不能改写成“完整稳定窗口策略已实现”。
...
| Quote `connecting/streaming` | 当前 generation 已建立或正在收候选                          | 可保留兼容旧值；首个 actionable 到达后切 current pin | picker 可用；AUTO / CUSTOM 与其余 gate 通过后可 Review；禁止 prebuild |
| Quote `settling`             | 已有 current pin，等待 quiet / all-terminal / hard deadline | 展示 pinned current quote                            | picker / Review 沿用 current pin；禁止 prebuild                       |
| Quote `committed`            | 权威 terminal final commit 已执行或判定无需更新             | 是                                                   | current pin 可 Review / Confirm                                       |
| Quote `refreshing`           | 同 fingerprint 的新 request 正在刷新                        | 旧 displayQuote 仅只读保留                           | 新 request 首个 actionable 到达前不可执行                             |
...
| `QUOTE_FIRST_ACTIONABLE_PINNED` | main quote reducer                    | current request / fingerprint、统一 actionability | 首次提交 display / executable；picker 可用；Review 继续检查其余 gate |
| `QUOTE_SETTLEMENT_REACHED`      | main quote reducer                    | current request、final commit 未提交              | 最多一次 final display / executable update；Frozen Review 不变       |
...
main 同步预读
  ...
  → 首个当前 actionable quote 立即 pin display + executable，开放 picker
  → effective AUTO / CUSTOM 与其余安全 gate 通过后可在 done 前进入 Frozen Review
  → terminal 最多一次 final commit，且不得改写 Frozen Review
  → 用户进入 Frozen Review
...
- `executableQuote`：当前 request / fingerprint 的首个 actionable pin，或手选 / terminal bounded update 后的当前值；完整 expiresAt 校验尚未落地，所以当前 Draft 不能把“未过期”列为已实现条件。

当前实现不再把 terminal 当作首次发布点：首个当前 actionable candidate 立即 pin display + executable。权威终止仍是 final commit 与 zero-provider / terminal error 的边界；长期目标再把 final commit 扩展为满足任一项：
...
- 自动可见提交每个 request 最多两次：首个 actionable pin 一次，terminal final commit 最多一次；两者之间 Provider 列表可更新，但同 Provider economic fields 不变。
- 手选 Provider 是显式用户 transition，可在其当前 actionable candidate 到达时换 pin；它不计入自动更新预算。若已选 Provider 变为不可执行，触发明确重新报价，不静默替换。
- Provider actionability 统一拒绝 error、非有限 / 非正输出、malformed / violated limits；BUY / SELL 都用 `fromAmount` 校验 min / max。
- SSE active 且 effective AUTO recommendation 未到时只阻断 early Review，不阻断 picker；effective CUSTOM 不等待 AUTO。Review 一旦冻结，手选、后续流与 terminal commit 都不能改写它。
- 周期刷新使用新 requestId；若 fingerprint 相同且旧 quote 未过展示 TTL，保留旧 displayQuote 并显示 `Refreshing`，但执行按钮依当前 quote 有效性决定是否禁用。
- fingerprint 改变时，旧金额不伪装成新交易对的金额。主输出稳定进入 `— / Updating`，等待新 generation 的首个 actionable candidate 建立 current pin。
...
| 首个当前 actionable candidate               | 切换到 pinned 新金额             | 不显示                     | picker 可用；Review 通过 AUTO/CUSTOM 与其余 gate 后可用 |
| 同 Provider 后续 streaming                  | 保持 pinned 经济字段             | 不显示                     | 不因 transport 未完成被重新锁住                         |
| terminal final commit                       | 最多再切换一次                   | 不显示                     | 已冻结 Review 不变化                                    |
...
### 10.1 Stock account slot、region owner 与 silent refresh

Stock display cache 的物理槽只按 account 划分，最多保留 8 个账户；不能再用一个 `account + stock + pay + side + currency` 总 identity 同时决定所有区域。每个区域按最小真实依赖建立 owner：

| Region       | Owner key                               | 可恢复内容 / 刷新合同                                            | 执行边界                                                          |
| ------------ | --------------------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------- |
| selection    | `account`                               | stock / pay / side 作为该账户最近展示选择                        | 只做冷启动 / 重入展示种子，不替代 live channel owner              |
| token detail | `account + stock + displayCurrency`     | header、price、change、logo；matching last-good 失败时保留       | cached market-open / description 不参与交易判断                   |
| balance      | `account + liveInputToken`              | 显示最近余额；canonical owner ready 后请求 exact live balance    | cached balance 不解锁 Max、百分比、quote、Review                  |
| chart        | `account + stock + sourceCurrency(usd)` | last-good data 静默刷新；空 / 失败有显式 phase                   | `visibleRange` 与 `requestedRange` 分离；旧数据不能贴新标签       |
| amount       | `account + stock + pay + side`          | 恢复最近输入，包括空字符串 tombstone；live owner ready 后再 seed | cached amount 在 canonical owner ready 前不可编辑、不可触发 quote |
...
- SSE active 的 early Review 必须关闭 prebuild；只冻结当前 pin，不能让后续流修改 snapshot。
- `expiresAt` 仍满足最小确认窗口。
...
| ----------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| 普通 Swap                     | Swap channel / SSE                  | 标准 `useSwapQuote` 接入 V2；首 actionable pin display / execution，AUTO/CUSTOM Review gate，同 Provider 稳定，terminal bounded update，raw intent 失效 | 真实账户切换、弱网、周期刷新、手选 Provider 全程录屏待回填   |
| 跨链 Bridge                   | Swap / Bridge capability / SSE      | 复用标准 V2 session 与冻结 Review 主路径                                                                                                                | BTC / EVM 多链实际 build、receiver 与 expiry 未完整证明      |
| Swap Modal                    | modal surface / SSE                 | 每个 hook instance 生成独立 surface；logical service registry 按 surface 隔离                                                                           | 主 Tab + Modal 同时活跃的目标平台 runtime 证据待回填         |
...
| Stock                         | Stock channel / SSE / market state  | account 物理槽 + region owners、cached amount/balance display-only、chart silent refresh、live canonical balance gate；quote path 使用 V2               | 五平台 Stock 首帧、切账户、range 与真实下单 runtime 未完成   |
...
| QUOTE-ERROR-THEN-SUCCESS-001  | 某 Provider 先错、其他 Provider 后成功     | 不提前进入全局 Error；首个 actionable 成为 pin          |
| QUOTE-OUT-OF-ORDER-001        | sequence 4 先于 3 到达                     | 3 被丢弃或按协议缓冲；不得回滚状态                      |
| QUOTE-DUPLICATE-001           | 同 Provider 重复 / 更新事件                | provider count 去重；streaming 经济字段不变             |
...
| QUOTE-REFRESH-001             | fingerprint 相同的周期刷新                 | 旧显示仅 display；首 actionable pin，terminal 最多一次  |
...
| QUOTE-AUTO-SLIPPAGE-001       | AUTO recommendation 晚于 Provider quote    | picker 可用；Review 等 recommendation；经济字段不变     |
| QUOTE-CUSTOM-SLIPPAGE-001     | session effective mode 为 CUSTOM           | 不受全局 AUTO / pending recommendation 误锁             |
| QUOTE-ACTIONABLE-LIMIT-001    | error / 非正输出 / malformed 或越界 limits | picker 与 Review 均 fail-closed；BUY/SELL 用 fromAmount |
...
| 每个 quote request 的自动主输出提交次数                             | ≤ 2：首 actionable pin + terminal 最多一次；中间 0 次 |
...
- quote display / execution：retained 旧值只展示；首个当前 actionable pin display + executable；同 Provider streaming 固定经济字段；手选显式切换；terminal 最多一次 final commit；AUTO recommendation / CUSTOM override 门闩；provider error / output / limit actionability；zero-provider 权威终态。
...
Case 跟踪表使用以下覆盖层标记：`U`（纯函数 / reducer）、`H`（Hook / Jotai）、`B`（logical service / EventSource 合同）、`I`（目标平台 runtime-boundary 集成）、`R`（真实 runtime）。所有 P0 case 至少具备 `U/H/B` 中适用的自动化层、`I` 和 `R`；不能把最终截图当成 `R` 的完整证据。
...
| readiness                           | [`swapQuoteReadiness.test.ts`](../packages/kit/src/views/Swap/utils/swapQuoteReadiness.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `PASS（统一矩阵）` |
| raw intent / projection             | [`quoteSemanticIntent.test.ts`](../packages/kit/src/states/jotai/contexts/swap/quoteSemanticIntent.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `PASS（统一矩阵）` |
| main quote identity                 | [`quoteSessionV2.test.ts`](../packages/kit/src/states/jotai/contexts/swap/quoteSessionV2.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `PASS（统一矩阵）` |
| bg ordinary session                 | [`ServiceSwapQuoteSession.test.ts`](../packages/kit-bg/src/services/ServiceSwapQuoteSession.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `PASS（统一矩阵）` |
| commit / UI phase / provider gate   | [`quoteCommittedState.test.ts`](../packages/kit/src/states/jotai/contexts/swap/quoteCommittedState.test.ts)、[`quoteProgress.test.ts`](../packages/kit/src/states/jotai/contexts/swap/quoteProgress.test.ts)、[`actions.test.tsx`](../packages/kit/src/states/jotai/contexts/swap/actions.test.tsx)、[`useSwapState.test.ts`](../packages/kit/src/views/Swap/hooks/useSwapState.test.ts)、[`SwapQuoteResult.test.tsx`](../packages/kit/src/views/Swap/pages/components/SwapQuoteResult.test.tsx)、[`swapProviderQuoteAvailability.test.ts`](../packages/kit/src/views/Swap/utils/swapProviderQuoteAvailability.test.ts)、[`swapQuoteSortUtils.test.ts`](../packages/shared/src/utils/swapQuoteSortUtils.test.ts)                       | `PASS（统一矩阵）` |
| Stock snapshot / amount / chart     | [`useIdentityScopedSilentRefresh.test.tsx`](../packages/kit/src/hooks/useIdentityScopedSilentRefresh.test.tsx)、[`swapStockDisplaySnapshotUtils.test.ts`](../packages/kit/src/views/Swap/hooks/swapStockDisplaySnapshotUtils.test.ts)、[`swapStockDisplaySnapshotStorage.test.ts`](../packages/kit/src/views/Swap/hooks/swapStockDisplaySnapshotStorage.test.ts)、[`useSwapStockDisplaySnapshot.test.tsx`](../packages/kit/src/views/Swap/hooks/useSwapStockDisplaySnapshot.test.tsx)、[`useSwapStockTradeInputs.test.ts`](../packages/kit/src/views/Swap/hooks/useSwapStockTradeInputs.test.ts)、[`SwapStockDesktopContainer.utils.test.ts`](../packages/kit/src/views/Swap/pages/components/SwapStockDesktopContainer.utils.test.ts) | `PASS（统一矩阵）` |
| balance / token detail owner        | [`tokenDetailRequest.test.ts`](../packages/kit/src/states/jotai/contexts/swap/tokenDetailRequest.test.ts)、[`actions.test.tsx`](../packages/kit/src/states/jotai/contexts/swap/actions.test.tsx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `PASS（统一矩阵）` |
| focus / overlay lifecycle           | [`swapQuoteFocusLifecycle.test.ts`](../packages/kit/src/views/Swap/hooks/swapQuoteFocusLifecycle.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `PASS（统一矩阵）` |
| Private Send                        | [`privateSendQuoteSession.test.ts`](../packages/kit/src/views/Send/pages/SendAmountInput/privateSendQuoteSession.test.ts)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `PASS（统一矩阵）` |
...
- 当前 hook / Jotai 自动化覆盖 start RPC reject、bg lease reject、exact close、reset 后 late event、Tab 切换、SELL / LIMIT BUY debounce 前失效、first-actionable pin、same-provider economic pin、bounded terminal commit、AUTO/CUSTOM gate、provider actionability、balance owner、Stock region owner / amount editability / chart range guard。
...
- 待补：quiet / hard deadline（当前未实现）、自动重连（当前未实现）、同时挂载 Tab + Modal 的目标平台 runtime-boundary 集成、统一 Visible Snapshot 投影（当前未实现）、expiry / service fingerprint / duplicate Confirm。

### 16.4 真机 / Runtime 复测

最终代码 runtime 结论：**Desktop dev Electron Partial PASS；production cold restart 与 Web / Extension / iOS / Android Pending-R**。复测使用新建隔离测试钱包；未读取、输出、截图或记录助记词。Slow 4G profile 为 562.5 ms latency、180,000 B/s download、84,375 B/s upload；独立 profile、测试服务和调试端口在结束后均已关闭或删除。

| 最终代码 Desktop dev real-surface 场景  | 观测到的状态序列                                                                                                                                                 | 结果 / 边界                                                                                                                                                                           |
| --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 首个 current actionable Provider        | 新输入后 retained 旧行立即变为 `aria-disabled=true`、`tabIndex=-1`；首个当前 Provider 约 583–796 ms 到达，约 583 ms 即可发起选择，后续 Provider 约 3.92 s 才到达 | **PASS**：无需等待 SSE `done`；选择后主金额保持 pin。测试钱包余额为 0，Review 按钮因余额门闩不可达，所以 early Review 仅由自动化覆盖，不能误报为 runtime PASS                         |
| 手选 Provider                           | 当前列表中手动选择另一 Provider 后，主金额按用户动作显式切换，后续同 Provider 更新没有再次跳动                                                                   | **PASS**：用户主动切换允许改变金额；后台 streaming 不得抢回                                                                                                                           |
| 普通 Swap → Stock                       | 普通 Swap 输入 `0.016` 后进入 Stock，Stock amount input 为独立空值；普通 Swap 数值未出现在 Stock                                                                 | **PASS**：跨模式 amount / balance 展示隔离                                                                                                                                            |
| Stock 展示快照与静默刷新                | 进入 Stock 后首个观测帧约 31 ms 已有 7 个 chart canvas、0 skeleton；cached AAPL header 约 659 ms 就位                                                            | **Partial PASS**：同次运行与 warm navigation 的 region snapshot 生效；production cold restart 尚待包级复测                                                                            |
| Chart `visibleRange` / `requestedRange` | 点击 `1M` 后，旧 `1W` 标签与 7 个 canvas 在约 819 ms 请求期保持；live 结果提交后标签与图表原子切到 `1M`                                                          | **PASS**：pending 期间不把 retained chart 误标为新 range                                                                                                                              |
| account-scoped 持久槽                   | 存储中观察到 <code>hd-1&#124;hd-1--0&#124;default</code> 账户槽，range 在提交后为 `1M`                                                                           | **代码 / 存储 PASS**；dev Electron 全重启因现有 `hydrate.ts` 开发策略跳过同步 hydration，仅预热 SWR，因此不能把 dev restart 当 production cold-start 失败，也不能宣称 production PASS |
...
最终代码 Runtime **没有覆盖**：账户 A→B、receiver 改变、token A→B→C、LIMIT BUY、Bridge、Root modal / 锁屏、Tab + Modal 并发、Private Send、Market Speed、可点击 Review drift、硬件 / QR / batch / wrap，以及 Web / Extension / iOS / Android；Stock production cold restart、账户切换和真实下单也仍待验证。开发控制台观察到 `User-Agent` unsafe-header 警告和本地 Agentation 连接失败，没有把它们归因为 Swap；发布前仍需生产包、干净 console 与按目标平台标注物理 topology 的结构化 timeline。

每个平台执行第 13 节核心 case，并保留：

- 启动方式、build / commit、账户类型、network、入口、网络条件。
- 屏幕录制和结构化状态时间线。
- UI 与 logical service owner 的 request / generation 对齐证据；仅 iOS / Android / Extension 要求跨 context envelope 证据，Desktop / Web 标注单 runtime。
...
| --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- |
| 完整 cold pair / All Networks                 | `SWAP-AN-001`、`SWAP-AN-PERSISTED-001`、`SWAP-PRESERVE-001`、`SWAP-PRESERVE-BTC-001`、`BOOT-PARTIAL-001`                                                                                                                                                                                       | `useSwapColdStartDisplayTokens.test.ts`：complete candidate、partial reject、no-mix、cold→live atomic switch                                   | process cold launch 录屏 + pair transition timeline                                              | Auto-core；历史 Desktop ETH/USDC baseline；最终 runtime Pending         |
...
| first-actionable / actionability / AUTO gate  | `QUOTE-FIRST-ACTIONABLE-001`、`QUOTE-PIN-STABILITY-001`、`QUOTE-ACTIONABILITY-001`、`QUOTE-AUTO-SLIPPAGE-001`、`QUOTE-CUSTOM-SLIPPAGE-001`、`QUOTE-ACTIONABLE-LIMIT-001`                                                                                                                       | `quoteCommittedState.test.ts`、`quoteProgress.test.ts`、`actions.test.tsx`、`useSwapState.test.ts`、`swapProviderQuoteAvailability.test.ts`    | 首 actionable 与 done 时序、same-provider 金额序列、AUTO/CUSTOM、BUY/SELL limits                 | Auto-core；Desktop picker early + pin PASS；Review / BUY Pending-R      |
| refresh / disconnect                          | `QUOTE-DISCONNECT-001`、`QUOTE-REFRESH-001`                                                                                                                                                                                                                                                    | `quoteProgress.test.ts`、`quoteCommittedState.test.ts` 覆盖保留 display / 清 executable                                                        | 已有报价时断网、周期刷新；金额 / skeleton commit 计数                                            | 历史 Desktop refresh baseline；最终 runtime / expiry / TTL Pending      |
...
| Stock display region / amount / chart         | `STOCK-DISPLAY-RESTORE-001`、`STOCK-DISPLAY-ISOLATION-001`、`STOCK-BALANCE-FAIL-CLOSED-001`、`STOCK-CHART-RANGE-001`、`STOCK-SWAP-BOUNDARY-001`                                                                                                                                                | `useIdentityScopedSilentRefresh.test.tsx`、snapshot utils/storage/hook、`useSwapStockTradeInputs.test.ts`、Container utils、`actions.test.tsx` | 普通 Swap → Stock 首帧、反向切换、账户切换、canonical owner 前编辑尝试、chart range 切换         | Auto-core；Desktop Swap→Stock、warm restore、range PASS；其余 Pending-R |
...
| 五平台                                        | `PLATFORM-DESKTOP-001`、`PLATFORM-WEB-001`、`PLATFORM-EXT-001`、`PLATFORM-IOS-001`、`PLATFORM-ANDROID-001`                                                                                                                                                                                     | 无法由 unit tests 替代                                                                                                                         | 第 14.4 节统一设备 / 网络 / cold-warm harness                                                    | Desktop dev 最终代码 Partial；完整五平台 benchmark 仍 Pending           |
...
### Stage A：收口当前 Draft（自动化与 Desktop dev 部分完成）

- 已完成：zero-provider、manual-provider、signed-order / late build、active-input fingerprint、同 key balance overlap 的最终 P0 / P1 交叉审计。
- 已完成：第 16.2 节 unified Jest（36 / 458）、35-file type-aware oxlint、`git diff --check` 与 `agent:check --profile commit`。
- Desktop dev 已完成：首 actionable picker、普通 Swap→Stock 隔离、warm Stock display/chart snapshot 与 range label；early Review、canonical owner 有余额对照、非空 Stock amount restore、production cold restart 及其余平台继续 Pending-R。
- PR profile 与远端 checks 不属于本次 commit 前占位，不得从本地静态门禁推导为通过。
...
| candidate / display / executable      | [`quoteCommittedState.ts`](../packages/kit/src/states/jotai/contexts/swap/quoteCommittedState.ts)、[`quoteProgress.ts`](../packages/kit/src/states/jotai/contexts/swap/quoteProgress.ts)、[`actions.ts`](../packages/kit/src/states/jotai/contexts/swap/actions.ts)、[`atoms.ts`](../packages/kit/src/states/jotai/contexts/swap/atoms.ts)、[`useSwapState.ts`](../packages/kit/src/views/Swap/hooks/useSwapState.ts)、[`swapProviderQuoteAvailability.ts`](../packages/kit/src/views/Swap/utils/swapProviderQuoteAvailability.ts)                                                                                                                           | first-actionable display/execution pin、same-provider economic freeze、AUTO/CUSTOM gate、actionability、bounded terminal、manual provider、exact reset |
| Stock display / silent refresh        | [`useIdentityScopedSilentRefresh.ts`](../packages/kit/src/hooks/useIdentityScopedSilentRefresh.ts)、[`swapStockDisplaySnapshotUtils.ts`](../packages/kit/src/views/Swap/hooks/swapStockDisplaySnapshotUtils.ts)、[`swapStockDisplaySnapshotStorage.ts`](../packages/kit/src/views/Swap/hooks/swapStockDisplaySnapshotStorage.ts)、[`useSwapStockDisplaySnapshot.ts`](../packages/kit/src/views/Swap/hooks/useSwapStockDisplaySnapshot.ts)、[`useSwapStockTradeInputs.ts`](../packages/kit/src/views/Swap/hooks/useSwapStockTradeInputs.ts)、[`SwapStockDesktopContainer.tsx`](../packages/kit/src/views/Swap/pages/components/SwapStockDesktopContainer.tsx) | account 物理槽 + region owners、cached amount/balance display-only、canonical owner edit gate、visible/requested chart range                           |
...
- [x] 代码 / 报告明确列出未实现能力：Boot coordinator、composite seed、Visible Snapshot、quiet / hard settlement、reconnect、expiry、service fingerprint、idempotency、telemetry、flags、五平台 benchmark；PR 描述创建后必须同步。
...
- [ ] first-actionable pin、bounded terminal / quiet / hard settlement、周期刷新、断线重连、zero-provider、manual-provider 均满足主金额提交与 skeleton re-entry 门槛。
...
5. **已落实 Quote 层**：UI 与 logical service owner 通过版本化、可验证的 V2 quote envelope 通信；iOS / Android / Extension 跨 context，Desktop / Web 在单 runtime 内本地分发；Build / execution envelope 尚未统一。
...
8. **已落实 first-actionable + bounded terminal 版本**：retained、candidate、pinned display / executable 分层；首个当前 actionable 早提交，同 Provider streaming 固定经济字段，terminal 最多一次 final commit；quiet / hard 策略未实现。
