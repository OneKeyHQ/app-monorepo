...
安全卡片模型本身不新增不可绕过的永久安全阻断。若未来在该模型中加入合规或策略性禁止，应新增独立 `blocked` 状态，不能复用 `critical`。Batch PSBT 页面目前仍继承上游对 High 站点的既有硬阻断；该页面级策略不由本模型的 `confirmation` 表达。
...
只基于请求类型的泛化提示，在可信站点的标准 Permit/Permit2 流程中不得单独成为风险结论。例如“Permit 签名可能导致资产损失”应与同一请求的结构化 Permit 核对项合并，只保留一条中性的 `info`。该去重仅适用于站点安全等级为 `security` 且不存在具体 parser 或地址风险的场景；未验证或风险站点继续保留泛化 warning。
