...
git コマンドラインツールで最新のコードを取得した後、プロジェクトの依存関係を `bun install` コマンドでルートディレクトリにインストールします。
...
- `bun run app:web`: ウェブモードを開発し、ポート 3000 の静的サーバをローカルに起動します。
- `bun run app:ios`: USB接続でiphoneデバイスの開発をデバッグする。
- `bun run app:android`: Androidのデバッグ
- `bun run app:desktop`: デスクトップモードでの開発
- `bun run app:ext`: ブラウザプラグインを開発します
...
サブディレクトリの依存関係をインストールする必要があるときは、`bun add axios --cwd apps/web`とすればよいでしょう。 `--cwd apps/web` のようなパラメータをつけると、最終的に axios モジュールは web サブプロジェクトのルートディレクトリにインストールされるようになります。
...
起動フェーズでの環境、モジュール、および依存関係の問題については、最初にルートディレクトリでコマンド`bun run clean`を使用することをお勧めします。このコマンドは、すべてのサブ依存関係、モジュールキャッシュ、metro / babelなどのツールのキャッシュをクリアしてから、プロジェクトを再起動して試行します。
