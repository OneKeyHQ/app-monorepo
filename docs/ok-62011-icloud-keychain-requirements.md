...
需求：[OK-62011](https://onekeyhq.atlassian.net/browse/OK-62011)。按讨论最终确认的“仅本机快捷解密，新设备先手输一次”实施。更新日期：2026-09-18。
...
自动化测试覆盖：真实缓存加解密与 AAD 绑定、跨账号相同 recordId 隔离、密文篡改、密钥缺失模拟换机、并发初始化、缓存读写失败、修改密码和历史备份隔离、恢复凭证过期及账号切换、备份授权/加密/上传/回读期间切换或退出账号、本地授权取消、导入失败、缓存命中/手输回退的界面调用顺序、Google Drive 原流程以及原有备份导出兼容性。

本次验证：`yarn agent:check --profile commit` 全部通过（含 lint、格式、TypeScript、后台接口约束）；7 组定向 Jest 测试共 45 项通过，包含新增缓存/恢复/界面测试及既有导出、SimpleDB 契约回归。
