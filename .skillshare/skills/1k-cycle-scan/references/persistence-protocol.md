...
> circuit breakers, Slack dedup, completeness, progress, hash/boundary
> asserts (`plan-run`), coverage-gap reconciliation (`reconcile`), comment
> bodies and Slack lines (`make-comment`), run report bodies
> (`make-report`), and state transitions are ALL computed by
> `scripts/coordinate.mjs`. The model's job is I/O only: dump comments to
> JSON (paginate to exhaustion), run the op, execute the returned
> instructions verbatim. The prose below is the spec the engine implements
> (kept for humans and reviewers); when in doubt, the engine's output wins.
...
Claim procedure:
1. Finish ALL preflight BEFORE posting any claim — pinned tree, manifest
   rebuild, and the `plan-run` op (which asserts the pin/manifest/rules
   hashes and the table boundary, and writes the groups plan). Keeps the
   claim-to-abort window minimal: a pre-claim failure needs no void.
...
1. Run page body ← the run report, generated by `--op make-report`
   (`updateConfluencePage`; rendering spec in the playbook). It aggregates
   ALL checkpoints on the run page, not just the groups you scanned yourself
   (a takeover report must include the dead runner's groups too).
...
`config.slack.channelId`, generated by `make-comment --kind slack-run` /
`--kind slack-summary` (never hand-written):
