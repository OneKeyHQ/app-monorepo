...
**Fix**: Kept self-heal opt-in (`perp` / `localTokens` / `localHistory` only) so user-authored and credential entities still fail loudly; shared `isUnreadableStorageValueError` (`UnknownError` + message `includes`); backoff retries (50/500/1000ms) before delete; `defaultLogger.app.storage.simpleDbUnreadableSelfHeal` local trail for export.
