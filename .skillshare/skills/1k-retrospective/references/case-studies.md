...
## Case: Classic wallpaper black/white conversion crushed photos and flickered polarity
**Date**: 2026-08-19 | **Platforms**: shared (web, desktop, mobile via `imageUtils.ts` / webembed bridge) — Classic/Classic1S/Mini/ClassicPure custom wallpaper upload
**Symptom**: Ordinary color photos uploaded as Classic-family wallpapers rendered mostly or fully black; near-solid-color/low-contrast crops rendered as scattered noise instead of a clean block; the same class of photo could randomly flip between normal and inverted ("negative") output between near-identical uploads.
**Root Cause**: `convertToBlackAndWhiteImageBase64` used a plain RGB average with a fixed 128 threshold and a whiteCount>50% invert heuristic. The average under-weights green and crushes saturated colors dark; the fixed threshold can't adapt to per-image contrast. Introducing a first attempt at an adaptive Otsu threshold then exposed a second bug: the invert heuristic's 50% decision boundary became noise-sensitive, because Otsu's threshold naturally tracks each image's own median.
**Fix**: Replaced the average with perceptual (weighted) luminance; added Otsu's method for a per-image threshold, skipped below a minimum luminance range so near-solid crops fall back to a fixed threshold instead of binarizing JPEG block noise; replaced the invert heuristic with `shouldInvertForMajorityWhite`, keeping the post-threshold white-ratio signal but adding a dead zone around 50% so ordinary photos stay stable while genuinely skewed images (e.g. line art) keep correct polarity. Does NOT fix the separate RGBA-transparency-renders-as-black issue (OK-issue #12923), which happens further upstream in the crop-confirm JPEG re-encode step (`ImageCrop.tsx`) before this code ever runs — tracked separately.
**Catchable by**: NEW — not covered; automatic image-processing algorithm output quality on synthetic/edge-case inputs (near-solid color, skewed histograms, ordinary photos near a decision boundary) has no existing checklist item and needed interactive visual comparison tools plus numeric simulation to catch, not a UI/state review
