...
- **CI**: set `ENABLE_FUNCTION_TRACE` on `daily-build`, `daily-build-dev`, `release-ios` or `release-android`. It builds the `qa-internal-function-trace` EAS profile (internal only, never store-submits). `ENABLE_STARTUP_PROFILE` still builds `qa-internal-startup-profile` without function tracing.
...
- Tracing overhead distorts `[StartupProfile.js]` timings; use `qa-internal-startup-profile` for module timings. The accepted trade-offs (ERROR log level, log rotation, duplicate worklet events) are documented at the top of `apps/mobile/plugins/functionTrace.js`.
