...
| STOCK-DISPLAY-RESTORE-001      | Enter Stock with an amount/header/chart -> leave or kill/restart -> re-enter with the same account and region owners                                                            | First frame restores the account slot's matching selection/amount/token-detail/balance/chart regions. Each refreshes silently; cached amount/balance stay non-editable and non-executable until canonical live readiness.                                                                                                                |
| STOCK-DISPLAY-ISOLATION-001    | Restore a Stock snapshot -> switch account, stock token, pay token, side, or currency while old requests are pending                                                            | The old amount/header/chart is rejected in the same frame and no late old patch appears. The new identity shows only its own snapshot or bounded loading/empty state.                                                                                                                                                                    |
| STOCK-BALANCE-FAIL-CLOSED-001  | Restore a Stock display snapshot while the exact live execution-balance request fails, retries, then succeeds or is retried manually                                            | Snapshot remains visible, but Max, percentage shortcuts, balance validation, and Review stay disabled until exact current-scope live balance is published; retries are bounded and stale retries cannot unlock the new scope.                                                                                                            |
| STOCK-SWAP-BOUNDARY-001        | Enter ordinary Swap with a visible amount/balance -> switch to Stock on another pair -> switch account or token while in Stock -> switch back                                   | The centralized transition clears shared amounts, target/Stock-owned balances, and old quote execution before each destination first frame. An ordinary Swap from-balance stays hidden in Stock and may reappear only if its account/token request owner is still exact; only a matching Stock snapshot may restore display-only values. |
...
| SWAP-QUOTE-STREAM-001          | Cold start or stale refresh -> enter Swap with delayed multi-provider SSE events -> receive the first actionable provider before `done`                                         | Skeleton or retained amount changes once to the guarded first actionable candidate; provider picker and Review become usable; later events do not rotate the main quote, and terminal settlement cannot mutate a frozen Review.                                                                                                          |
...
15. For `SWAP-QUOTE-STREAM-001`, record the first frame, first actionable SSE
    candidate time, provider-picker first-click time, terminal `done` time,
    retained/main amount transitions, selected provider, and the exact moment
    Review becomes enabled. A pass requires the first current actionable quote
    to pin display/execution before `done`, no same-provider economic changes
    while streaming, at most one terminal update, and no mutation of the frozen
    Review. Record whether effective AUTO waited for its recommendation or a
    CUSTOM override was active.
16. For `STOCK-DISPLAY-RESTORE-001`, record the physical storage/runtime owner,
    account slot plus each region owner, snapshot timestamps, amount editability,
    chart visible/requested ranges, first restored frame, each live replacement
    time, and skeleton transitions. A pass has zero matching-region
    snapshot-to-skeleton round trips and no cached execution enablement.
17. For `STOCK-DISPLAY-ISOLATION-001`, record every changed identity field and
    prove that neither the old snapshot nor a late old async patch appears for
    one frame under the new account/pair/side/currency.
18. For `STOCK-BALANCE-FAIL-CLOSED-001`, record Max/percentage/Review state
    before failure, through every bounded retry, after explicit Retry, and only
    after an exact live balance plus matching shared-atom publication.
19. For `STOCK-SWAP-BOUNDARY-001`, record ordinary Swap and Stock pair/amount/
    selected-balance atoms immediately before and after both transitions. A
    pass has zero frame where one surface displays the other surface's amount,
    balance, or executable quote.
