...
| STOCK-SWAP-BOUNDARY-001        | Enter ordinary Swap with a visible amount/balance -> switch to Stock on another pair -> switch back                                                                             | The centralized type transition clears shared amounts, selected balances, and old quote execution before each destination first frame. Only a matching destination snapshot may restore display-only values.                    |
