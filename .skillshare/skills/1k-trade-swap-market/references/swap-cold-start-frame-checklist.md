...
| HOME-TOKEN-STOCK-001           | Wallet Home token list -> click a stock-capable token row quick action that opens the Swap/Stock modal                                                                          | The modal route, visible tab, imported token payload, and internal/effective semantic type agree. A Stock entry stays Stock-owned; ordinary Swap default-token sync must not rewrite the stock/pay-token context.          |
...
15. For `HOME-TOKEN-STOCK-001`, the token-row action source, imported token
    payload, modal route, first visible tab, internal/effective semantic type,
    and whether shared Swap default-token sync rewrote the Stock context.
