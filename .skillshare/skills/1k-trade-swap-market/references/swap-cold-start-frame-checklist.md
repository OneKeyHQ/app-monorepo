...
| STOCK-DISPLAY-RESTORE-001      | Enter Stock with a settled amount/header/chart -> leave or kill/restart -> re-enter with the same account, stock, pay token, side, and currency                                 | First frame restores the exact account-scoped snapshot. Token detail, balance, and chart refresh silently and independently; failure retains display, and no snapshot -> skeleton -> live round trip occurs.                  |
...
| SWAP-QUOTE-STREAM-001          | Cold start or stale refresh -> enter Swap with delayed multi-provider SSE events -> receive the first actionable provider before `done`                                         | Skeleton or retained amount stays stable; provider picker becomes usable on the first actionable current-request candidate; main amount/rate and Review/build remain unchanged until terminal settlement.                     |
...
15. For `SWAP-QUOTE-STREAM-001`, record the first frame, first actionable SSE
    ...
    Review/build becomes enabled. A pass requires selection before `done` with
    no pre-settlement display or execution commit.
16. For `STOCK-DISPLAY-RESTORE-001`, record the physical storage/runtime owner,
    logical identity, snapshot region timestamps, first restored frame, each
    live replacement time, and the number of skeleton transitions. A pass has
    zero same-identity snapshot-to-skeleton round trips.
