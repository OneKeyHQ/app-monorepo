...
| Quote readiness        | What independently unlocks provider selection, first-current display/execution pin, Review safety, and transport completion? |
| Manual provider intent | What stable provider identity is recorded during streaming, and how is it rebound to the active current candidate?           |
...
- actionable candidate: the exact field predicate that makes a provider row
  selectable: no `errorMessage`, finite positive `toAmount`, and well-formed
  min/max bounds checked against `fromAmount` for both BUY and SELL
...
- display readiness: retained prior display until the first actionable active-
  request candidate, then pin that amount/rate/provider instead of rotating on
  every later event; same-provider streaming may enrich non-economic fields,
  explicit manual selection may change the pin, and one terminal update is the
  automatic upper bound
- Review readiness: an actionable candidate from the active request/fingerprint
  can be frozen before settlement; retained stale display is never executable;
  while streaming, effective AUTO additionally requires its recommendation for
  early Review, while effective CUSTOM does not; use the effective session
  override and keep prebuild disabled until the stream completes
- execution readiness: the frozen candidate plus account, receiver, balance,
  slippage, expiry, and signer guards control build/sign/send
- manual provider intent: store provider identity during streaming and rebind
  it to the active candidate; never rely on object identity
...
Do not make one loading boolean own picker, display, Review/execution, and
transport-completion readiness.
