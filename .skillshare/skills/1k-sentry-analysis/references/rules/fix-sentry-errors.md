...
**The Challenge**: Correlation ≠ Causation
- ❌ "There were network requests AND UI froze" (weak - just correlation)
- ✅ "Network requests CAUSED UI freeze because..." (strong - proven causation)
...
┌─────────────────┐         ┌──────────────────┐
│  JavaScript     │◄───────►│   Native/Main    │
│  Thread         │  Bridge │   Thread (iOS)   │
│  (Event Loop)   │         │   (UI Updates)   │
└─────────────────┘         └──────────────────┘

Architectural Constraint:
- Bridge is serialized (messages cross one at a time)
- Network callbacks must cross bridge: Native → JS
- UI updates must cross bridge: JS → Native
...
1769745698.196 - First request starts
    ├─ 1769745698.196 [http] GET networkId=evm--56
    ├─ 1769745698.208 [http] GET networkId=evm--1   (+12ms)
    ├─ 1769745698.231 [http] GET networkId=evm--10  (+35ms)
    └─ 1769745698.303 [http] GET networkId=evm--146 (+107ms)
         │
         └─ 15 requests in 107ms window ← Peak concurrency
...
1. JS → Native: fetch() call                [1 message]
2. Native → JS: Response headers            [1 message]
3. Native → JS: Response body chunks        [2-3 messages]
4. Native → JS: Response complete           [1 message]
5. JS → Native: setState() update UI        [1 message]
───────────────────────────────────────────
Total: ~6 messages per request

With 15 concurrent requests:
15 requests × 6 messages = 90 bridge messages

Bridge Processing Capacity (iPhone 7):
- Processing Rate: ~1000 messages/second
- Large JSON parsing: 10-20ms per response
- 15 requests × 50 tokens × 15ms = 11,250ms of JS work
...
→ Main thread BLOCKED waiting for bridge to clear
...
Step 1: Concurrent requests fire
   ↓ [Evidence: Breadcrumbs show 15 requests in 107ms]

Step 2: Each request generates callbacks
   ↓ [Evidence: Network API architecture]

Step 3: Callbacks queue in bridge
   ↓ [Evidence: React Native bridge is serialized]

Step 4: Bridge processing delays UI updates
   ↓ [Evidence: UI updates also use bridge]

Step 5: Main thread waits for UI update messages
   ↓ [Evidence: Stack trace shows main thread in layout]

Step 6: Auto Layout blocks for >5 seconds
   ↓ [Evidence: iOS Watchdog 5000ms threshold]

Step 7: iOS Watchdog triggers AppHang
   ✓ [Evidence: Sentry AppHang event]
...
CAUSE (Observable Event)
    ↓ [Proof Type 1: Architecture allows this]
MECHANISM (How it works)
    ↓ [Proof Type 2: Timing shows correlation]
    ↓ [Proof Type 3: Math shows capacity exceeded]
INTERMEDIATE EFFECT
    ↓ [Proof Type 4: Stack trace confirms state]
    ↓ [Proof Type 5: Documentation validates pattern]
    ↓ [Proof Type 6: Mechanism explains connection]
...
15+ Concurrent Network Requests (CAUSE)
    ↓ [Architectural Proof: RN bridge is serialized]
90+ Bridge Messages Generated (MECHANISM)
    ↓ [Quantitative Proof: 15 × 6 = 90 messages]
Bridge Queue Saturation (EFFECT 1)
    ↓ [Temporal Proof: 15 requests in 107ms]
JavaScript Event Loop Backlog (EFFECT 2)
    ↓ [Quantitative Proof: 11,250ms of work in 107ms]
Main Thread Cannot Update UI (EFFECT 3)
    ↓ [Stack Trace Proof: Main thread in Auto Layout]
Auto Layout Blocks for 5+ Seconds (EFFECT 4)
    ↓ [External Validation: RN perf docs confirm]
💥 iOS Watchdog AppHang (FINAL EFFECT)
    ✓ [System Behavior: Watchdog 5s threshold]
```
...
→ Missing: HOW do network requests cause hanging?
```

❌ **Single type of evidence**:
```
"The timing matches" (only temporal correlation)
→ Missing: Architecture, quantitative, mechanism proofs
```

❌ **Ignoring alternative causes**:
```
"Must be the network requests"
→ Missing: Could it be memory, CPU, or other factors?
```

✅ **Strong causation proof**:
```
...
1. **Non-obvious relationships**: Network → UI stuttering
2. **Async/race conditions**: SDK cleanup → null reference
3. **Performance issues**: Multiple factors could be responsible
4. **Cross-system effects**: JS thread → Native thread → UI
