...
describe('Nostr Crypto Functions', () => {
  ...
  const testPublicKey = '02a8b5e5163c1d78754dd9229a42047f3ff4b069b99868580da3bb915960e7e9d8';
  const testPlaintext = 'Hello, Nostr!';

  it('should match snapshot for encryption', async () => {
    const encrypted = await encrypt(testPrivateKey, testPublicKey, testPlaintext);
  ...
  it('should validate encryption format', async () => {
    ...
    expect({
      ...
      hasValidFormat: encrypted.match(/^[A-Za-z0-9+/]+=*\?iv=[A-Za-z0-9+/]+=*$/) !== null
    }).toMatchSnapshot();
  });
});
