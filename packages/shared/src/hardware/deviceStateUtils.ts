...
export function resolveHardwarePassphraseEnabled({
  features,
}: {
  features: IOneKeyDeviceFeatures;
}) {
  return Boolean(
    features.passphrase_protection ?? features.passphraseProtection,
  );
}
...
function sanitizeState(state: IOneKeyDeviceState) {
  ...
  delete (
    nextState.identity as unknown as {
      displayName?: unknown;
    }
  ).displayName;
  return nextState;
...
export function projectLegacyDeviceFeaturesFromState(
  ...
  return {
    ...
    seVersion: versions.se ?? versions.se01,
    se01Version: versions.se01 ?? versions.se,
