...
// The Headless SDK is native-iOS-only. The extension always reports unsupported
// so the branch helper routes to the existing web widget.
export function canUseHeadless(): boolean {
  return false;
}

// Never called on the extension (the page is native-only); present so the shared
// import resolves on every platform.
