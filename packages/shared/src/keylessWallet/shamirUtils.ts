/* eslint-disable spellcheck/spell-checker */
import { combine, split } from 'shamir-secret-sharing';

// GF(256) arithmetic for Shamir secret recovery
// Using the same lookup tables as shamir-secret-sharing library
// Polynomial: x^8 + x^4 + x^3 + x + 1, Generator: 0xe5 (229)

import { OneKeyLocalError } from '../errors';
import bufferUtils from '../utils/bufferUtils';

// Tables from: https://github.com/hashicorp/vault/blob/9d46671659cbfe7bbd3e78d1073dfb22936a4437/shamir/tables.go
const GF256 = (() => {
  // Provides log(X)/log(g) at each index X
  const LOG = new Uint8Array([
    ,
  ]);

  // Provides the exponentiation value at each index X
  const EXP = new Uint8Array([
    ,
  ]);

  // eslint-disable-next-line no-bitwise
  const xor = (a: number, b: number): number => a ^ b;

  return {
    add: (a: number, b: number): number => xor(a, b),
    sub: (a: number, b: number): number => xor(a, b),
    mul: (a: number, b: number): number => {
      if (a === 0 || b === 0) return 0;
      return EXP[(LOG[a] + LOG[b]) % 255];
    },
    div: (a: number, b: number): number => {
      if (b === 0) {
        throw new OneKeyLocalError('GF256 division by zero');
      }
      if (a === 0) return 0;
      return EXP[(LOG[a] - LOG[b] + 255) % 255];
    },
  };
})();

/**
 * Recover the missing Shamir share using GF(256) arithmetic.
 * Given the secret (entropy) and one share, compute any other share.
 *
 * Share format (shamir-secret-sharing library): [y-values (N bytes), x-coordinate (1 byte)]
 * Note: x-coordinate is at the END, not the beginning!
 *
 * Math: f(x) = secret + a1*x in GF(256) for threshold=2
 * Given secret and (x1, y1), compute a1 = (y1 - secret) / x1
 * Then compute y_missing = secret + a1 * x_missing
 */
function recoverMissingShare(params: {
  entropyHex: string; // mnemonicToEntropy(mnemonic);
  shareBase64: string; // base64 encoded share
  // The x-coordinate of the missing share (must be extracted from original share)
  missingX: number;
}): string {
  const { shareBase64, entropyHex, missingX } = params;
  const shareBytes = bufferUtils.base64ToBytes(shareBase64);
  // Use toBuffer (same as generateKeylessMnemonic) to ensure consistent entropy bytes
  const entropyBytes = bufferUtils.toBuffer(entropyHex);

  // Share format: [y-values (N bytes), x-coordinate (1 byte)]
  // x-coordinate is the LAST byte
  const secretLength = shareBytes.length - 1;
  const x1 = shareBytes[secretLength]; // x-coordinate at the end
  const y1 = shareBytes.slice(0, secretLength); // y-values at the beginning

  // Compute y-values for the missing share
  const missingY = new Uint8Array(secretLength);

  for (let i = 0; i < secretLength; i += 1) {
    const s = entropyBytes[i]; // secret byte
    const yi = y1[i]; // share1's y-value for this byte

    // a1 = (y1 - s) / x1 in GF(256)
    // In GF(256), subtraction is XOR
    const a1 = GF256.div(GF256.sub(yi, s), x1);

    // missingY = s + a1 * missingX
    missingY[i] = GF256.add(s, GF256.mul(a1, missingX));
  }

  // Construct the missing share: [y-values, x-coordinate]
  // Same format as shamir-secret-sharing library
  const missingShareBytes = new Uint8Array(secretLength + 1);
  missingShareBytes.set(missingY, 0);
  missingShareBytes[secretLength] = missingX;

  return bufferUtils.bytesToBase64(missingShareBytes);
}

export default {
  GF256,
  split,
  combine,
  recoverMissingShare,
};
