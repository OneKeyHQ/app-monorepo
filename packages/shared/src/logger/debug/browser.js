...
async function save(namespaces) {
  try {
    if (namespaces) {
      await exports.storage.setItem(storageKey, namespaces);
    } else {
      await exports.storage.removeItem(storageKey);
    }
  } catch (error) {
    // Swallow
...
async function load() {
  let r;
  try {
    r = await exports.storage.getItem(storageKey);
  } catch (error) {
    // Swallow
    // XXX (@Qix-) should we be logging these?
    console.error('debug logger storage error', error);
