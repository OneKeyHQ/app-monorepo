...
//
// 2. The indexer is the ONLY source for the public half. There is deliberately
//    no fallback to the local scan: the scanner has no product claim on public
//    funds, and substituting its view would present stale data as current.
//    When the indexer cannot be asked, the total is reported incomplete
//    (`balanceStatus`) rather than silently shrunk.
...
export function composePrivacyChainBalance({
  ...
  const publicSide = new BigNumber(publicSideIndexer ?? '');
