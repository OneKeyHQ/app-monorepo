...
import { Stacks } from './sdk';
import settings from './settings';
import {
  buildDecodeTxFromTx,
  estimateFee,
  estimateSize,
  getStacksNetworkInfo,
  verifyStacksAddress,
  verifyStacksAddressPrefix,
} from './utils';
...
import { publicKeyToAddress, StacksTransaction } from '@stacks/transactions';
...
export default class Vault extends VaultBase {
  ...
  override async getDisplayAddress(address: string): Promise<string> {
    if (verifyStacksAddressPrefix(address)) {
      return address;
    }
  ...
  createSDKClient = memoizee(
    async (rpcUrl: string, networkId: string, isTestnet: boolean) => {
      const sdkClient = this.createClientFromURL(rpcUrl) as Stacks;
      const chainInfo =
    ...
    },
    {
      ...
      normalizer(
        args: Parameters<
          (rpcUrl: string, networkId: string) => Promise<Stacks>
        >,
      ): string {
  ...
  async getSDKClient(): Promise<Stacks> {
    const { rpcURL, isTestnet } = await this.getNetwork();
    return this.createSDKClient(rpcURL, this.networkId, isTestnet);
  }
  ...
  override async validateWatchingCredential(input: string): Promise<boolean> {
    if (this.settings.watchingAccountEnabled) {
      if (input.startsWith('stacks')) {
        return Promise.resolve(verifyStacksAddress(input).isValid);
      }
      return verifyStacksAddress(await this.getDisplayAddress(input)).isValid;
    }
    return Promise.resolve(false);
  }

  override async validateAddress(address: string): Promise<string> {
    const { isValid, normalizedAddress } = verifyStacksAddress(address);
    if (isValid) {
  ...
  override async attachFeeInfoToEncodedTx(params: {
    ...
    // TODO set fees
    const tx = params.encodedTx as StacksTransaction;
    const fee = params.feeInfoValue.feeRate
      ? params.feeInfoValue.feeRate
      : 10_000;
    tx.setFee(fee);
    return Promise.resolve(tx as IEncodedTx);
  }
  ...
  ): Promise<IEncodedTxStacks> {
    const client = await this.getSDKClient();
    const fromStacksAddress = transferInfo.from;
    const utxos = (await client.getStacksUTXOs(fromStacksAddress)).filter(
  ...
  async buildDecodeTx(txHash: string): Promise<IDecodedTx | false> {
    const client = await this.getSDKClient();
    let tx: IStacksTransaction;
    try {
      tx = await client.getTransaction(txHash);
    } catch (error) {
      // The result from Stacks Transaction API may be incomplete JSON, resulting in parsing failure.
      debugLogger.common.error(`Failed to fetch Stacks transaction. `, txHash);
      return false;
    }
    ...
    const network = getStacksNetworkInfo(chainId);
    const token: Token = await this.engine.getNativeTokenInfo(this.networkId);
