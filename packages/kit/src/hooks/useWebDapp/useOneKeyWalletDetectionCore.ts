...
import type { EIP6963ProviderDetail, Store } from 'mipd';

const ONE_KEY_RDNS_PATTERNS = ['so.onekey.app.wallet'];

// ---------------------------------------------------------------------------
// Module-level subscription glue for useSyncExternalStore.
// Each platform variant calls `initMipdGlue(store)` once at module scope
// to wire up the shared cached-providers state.
// ---------------------------------------------------------------------------
...
export function initMipdGlue(store: Store | undefined) {
  sharedStore = store;
  cachedProviders = store?.getProviders() ?? [];
  ...
  const getOneKeyConnectionInfo =
    useCallback((): IExternalConnectionInfo | null => {
      ...
      const provider =
        ...
        findOneKeyProvider(sharedStore?.getProviders() ?? []);
