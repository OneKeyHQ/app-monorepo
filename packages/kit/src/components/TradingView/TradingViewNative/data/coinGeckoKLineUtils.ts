...
export function getCoinGeckoChartDays({
  timeFrom,
  timeTo,
  ...
  if (timeSpan <= ONE_DAY_SECONDS) {
    return '1';
  }
  if (timeSpan <= 7 * ONE_DAY_SECONDS) {
    return '7';
  }
  if (timeSpan <= 30 * ONE_DAY_SECONDS) {
    return '30';
  }
  if (timeSpan <= 365 * ONE_DAY_SECONDS) {
    return '365';
  }
  return 'max';
}
...
export function convertCoinGeckoChartToKLineResponse({
  ...
  const normalizedPoints = normalizeCoinGeckoChartData({
    chartData,
    timeFrom,
    timeTo,
  }).map(([timestamp, price]) => ({
