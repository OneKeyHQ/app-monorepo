...
const PRICE_EXTREMA_TEXT_BASELINE_OFFSET = PRICE_EXTREMA_FONT_SIZE / 2 - 1;
...
interface IChartPointPictureData {
  pricePicture: SkPicture;
  volumeMinimumPicture: SkPicture;
  volumePicture: SkPicture;
}
...
function createChartPointPictures({
  baseMaxPrice,
  baseMaxVolume,
  basePriceRange,
  chartType,
  ...
  indexedPoints: IIndexedChartPoint[];
  pointCount: number;
  priceAxisX: number;
  priceChartHeight: number;
  volumeBottom: number;
  volumeHeight: number;
}): IChartPointPictureData {
  ...
  if (chartType !== 'line') {
    indexedPoints.forEach(({ index, point }) => {
      ...
      pricePaint.setColor(skColor);
      pricePaint.setStrokeWidth(TRADING_VIEW_NATIVE_CANDLE_WICK_WIDTH);
      priceCanvas.drawLine(x, highY, x, Math.max(lowY, highY + 1), pricePaint);
...
function createChartPictures({
  ...
  const pointPictures = createChartPointPictures({
    baseMaxPrice,
    baseMaxVolume,
...
/* oxlint-disable react/style-prop-object -- Skia uses string paint styles. */
function TradingViewNativeLine({
  chartWidth,
  color,
  maxPrice,
  minPrice,
  ...
  const linePath = usePathValue((path) => {
    ...
    const lineEndIndex = Math.min(visiblePointRange.endIndex + 1, pointCount);
  ...
  const latestPointPosition = useDerivedValue(() => {
    const latestPoint = points[pointCount - 1];
    const position = latestPoint
      ? getTradingViewNativeLinePointPosition({
          candleGap: NATIVE_CANDLE_GAP,
          index: pointCount - 1,
          maxPrice: maxPrice.value,
  ...
  const latestPointOpacity = useDerivedValue(
    () => latestPointPosition.value.opacity,
  );
  const latestPointTransform = useDerivedValue(() => [
    { translateX: latestPointPosition.value.x },
    { translateY: latestPointPosition.value.y },
  ]);
  ...
  return (
    <>
      <Path
        ...
        style="stroke"
      />
      <Group opacity={latestPointOpacity} transform={latestPointTransform}>
        <Circle
          color={color}
          cx={0}
          cy={0}
          r={TRADING_VIEW_NATIVE_LINE_POINT_RADIUS}
        />
      </Group>
    </>
  );
...
function TradingViewNativePriceExtremum({
  chartOpacity,
  color,
  font,
  maxPrice,
  ...
  const layout = useDerivedValue(() => {
    ...
    const horizontalLayout = getTradingViewNativePriceExtremumHorizontalLayout({
      anchorX,
      canvasWidth: width,
      textWidth,
    });
    const y = getTradingViewNativePriceY(price, {
      maxPrice: maxPrice.value,
      priceChartHeight,
      priceRange: maxPrice.value - minPrice.value,
    });
  ...
  const lineStart = useDerivedValue(() => ({
    x: layout.value.anchorX,
    y: layout.value.y,
  }));
  const lineEnd = useDerivedValue(() => ({
    x: layout.value.lineEndX,
    y: layout.value.y,
  }));
  const textX = useDerivedValue(() => layout.value.textX);
  const textY = useDerivedValue(
    () => layout.value.y + PRICE_EXTREMA_TEXT_BASELINE_OFFSET,
  );
  const opacity = useDerivedValue(() => {
    const hasVisiblePriceRange =
      Number.isFinite(maxPrice.value) &&
      Number.isFinite(minPrice.value) &&
      maxPrice.value >= minPrice.value;
    const halfCandleWidth =
      (TRADING_VIEW_NATIVE_CANDLE_BODY_WIDTH * zoomScale.value) / 2;
    const isPointVisible =
      layout.value.anchorX >= CHART_HORIZONTAL_PADDING - halfCandleWidth &&
      layout.value.anchorX <= priceAxisX + halfCandleWidth;
    return hasVisiblePriceRange && isPointVisible ? chartOpacity : 0;
  });

  return (
    <Group opacity={opacity}>
      <Line color={color} p1={lineStart} p2={lineEnd} strokeWidth={1} />
      <Text color={color} font={font} text={text} x={textX} y={textY} />
    </Group>
  );
}
...
export const TradingViewNativeChart = memo(
  ({
    ...
    const lineColor = theme.text.val;
    ...
    const chartLegend = legendPoint
      ? getTradingViewNativeChartLegend(legendPoint, chartType)
      : null;
    let legendPointColor = lineColors.line;
    if (chartType !== 'line') {
      legendPointColor = chartLegend?.isUp ? CHART_UP_COLOR : CHART_DOWN_COLOR;
    }
    let latestPointColor =
    ...
    const picturePointsSnapshot = useMemo(
      ...
      [chartPictureVersion, chartType, points],
    );
    useLayoutEffect(() => {
    ...
    const latestPointPictureData = useMemo(() => {
      ...
      return createChartPointPictures({
        ...chartSize,
    ...
    const defaultVisiblePriceRange = useMemo(
      ...
      [chartType, defaultVisiblePointRange, points],
    );
    ...
    const visiblePriceExtrema = useMemo(
      () =>
        getTradingViewNativePriceExtrema({
          ...visiblePointRange,
          points,
        }),
      [points, visiblePointRange],
    );
        ...
        {chartPictureData ? (
          <GestureDetector gesture={chartGestures}>
            <Canvas testID={testID} style={{ flex: 1 }}>
              ...
              {visiblePriceExtrema ? (
                <>
                  <TradingViewNativePriceExtremum
                    ...
                    panOffset={panOffset}
                    pointCount={pointCount}
                    pointIndex={visiblePriceExtrema.high.index}
                    price={visiblePriceExtrema.high.price}
                    priceAxisX={priceAxisX}
                    priceChartHeight={chartPictureData.priceChartHeight}
                  ...
                  <TradingViewNativePriceExtremum
                    ...
                    panOffset={panOffset}
                    pointCount={pointCount}
                    pointIndex={visiblePriceExtrema.low.index}
                    price={visiblePriceExtrema.low.price}
                    priceAxisX={priceAxisX}
                    priceChartHeight={chartPictureData.priceChartHeight}
                    width={chartSize.width}
                    zoomScale={zoomScale}
