...
class AppUpdates {
  async checkAppUpdate(test?: {
    testMode?: boolean;
    testUpdate?: boolean;
  }): Promise<VersionInfo | undefined> {
    const testUpdateMode = (test?.testMode && test?.testUpdate) ?? false;
    return this._checkAppUpdate(testUpdateMode);
  }
  ...
  async _checkAppUpdate(always: boolean): Promise<VersionInfo | undefined> {
    ...
    if (platformEnv.isAndroid) {
      if (platformEnv.isAndroidGooglePlay) {
  ...
  async getChangeLog(oldVersion: string, newVersion: string): Promise<string> {
    const releaseInfo = await getReleaseInfo();
