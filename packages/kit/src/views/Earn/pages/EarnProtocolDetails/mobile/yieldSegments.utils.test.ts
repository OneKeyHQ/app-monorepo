  ...
  it('requires both the header and a fully classified item list', () => {
    expect(
      isYieldSheetAvailable({
        ...
        items: [
          { title: { text: 'Base' }, value: '+6.10%', kind: 'base', rate: '6.10' },
        ],
      }),
    ).toBe(true);
  });
...
describe('buildYieldSegments', () => {
  it('keeps yield rows and drops the fee row', () => {
    expect(
      buildYieldSegments([
        { title: { text: 'Base' }, value: '+6.10%', kind: 'base', rate: '6.10', color: '$a' },
        { title: { text: 'Bonus' }, value: '+2.12%', kind: 'campaign', rate: '2.12', color: '$b' },
  ...
  it('falls back to a neutral colour when the server sends none', () => {
    expect(
      buildYieldSegments([
...
describe('formatCountdown', () => {
  it('breaks the remainder into d/h/m/s', () => {
    ...
    expect(formatCountdown(9 * oneDay + 12 * 3_600_000 + 23 * 60_000 + 12_000)).toEqual({
      days: 9,
      hours: 12,
      minutes: 23,
      seconds: 12,
