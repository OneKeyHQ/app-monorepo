...
describe('buildPortfolioClaimSymbolMap', () => {
  ...
  it('does not guess when the same identity has conflicting symbols', () => {
  ...
  it('keeps non-EVM vault identities case-sensitive', () => {
    ...
    expect(
      buildPortfolioClaimSymbolMap([normalAsset]).get(
        getPortfolioProtocolIdentityKey(airdropAsset),
      ),
    ).toBeUndefined();
  });
...
describe('resolvePortfolioClaimProtocolIdentity', () => {
  it('uses the server-derived claim identity before the source position', () => {
    expect(
      resolvePortfolioClaimProtocolIdentity({
        isAirdrop: true,
        providerName: 'Morpho',
  ...
  it('preserves the Ethereum Morpho claim request identity', () => {
    expect(
      resolvePortfolioClaimProtocolIdentity({
        isAirdrop: true,
        providerName: 'Morpho',
  ...
  it('keeps a normal Morpho asset symbol without provider hardcoding', () => {
    expect(
      resolvePortfolioClaimProtocolIdentity({
        isAirdrop: false,
        providerName: 'Morpho',
    ...
    ).toEqual({ symbol: 'USDT', vault: 'morpho-usdt-vault' });
  });
  ...
  it('fails closed when the claim identity is ambiguous', () => {
    expect(
      resolvePortfolioClaimProtocolIdentity({
        ...
        sourceIdentity: { symbol: 'USDC', vault: 'shared-vault' },
      }),
    ).toBeNull();
  });

  it('preserves the Pendle reward protocol identity', () => {
    expect(
      resolvePortfolioClaimProtocolIdentity({
        isAirdrop: true,
        providerName: 'Pendle',
    ...
    ).toEqual({ symbol: 'USDe', vault: 'pendle-reward-vault' });
  });

  it('keeps a unique source position identity for unmatched airdrops', () => {
    ...
    ).toEqual({ symbol: 'slisBNB', vault: 'position-vault' });
  });
