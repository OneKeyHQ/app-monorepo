...
import BluetoothLottie from '@onekeyhq/kit/assets/wallet/lottie_connect_onekey_by_bluetooth.json';
import USBLottie from '@onekeyhq/kit/assets/wallet/lottie_connect_onekey_by_usb.json';
import {
...
import { ScannedDevice } from '@onekeyhq/kit/src/utils/device/deviceUtils';
...
const ConnectHardwareModal: FC = () => {
  ...
  }, [devices, handleConnectDeviceWithDevice, isConnectingDeviceId]);

  // Mobile Connect Screen
  const connectMobileScreen = useMemo(() => {
    if (!isSearching) {
      return (
    ...
    return (
      <VStack space={12} w="full" alignItems="center">
        <Box w="358px" h="220px" mb={-4}>
          <LottieView source={BluetoothLottie} autoPlay loop />
        </Box>
  ...
  }, [intl, isSearching, renderDevices]);

  const content = Platform.isNative ? (
    <Center>{connectMobileScreen}</Center>
  ) : (
    <VStack space={8} alignItems="center">
      <Box>
        <LottieView source={USBLottie} autoPlay loop />
      </Box>
