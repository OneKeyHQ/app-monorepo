...
export function useDeviceStageFirmwareVerify() {
  ...
  const runDeviceStageFirmwareVerify = useCallback(
    async ({
      ...
      try {
        useNewProcess = await serviceHardware.shouldAuthenticateFirmwareByHash({
          features,
        });
      } catch {
        useNewProcess = false;
      ...
      const runOnce = async (): Promise<IRunOutcome> => {
        ...
        try {
          ...
          if (!authResult.verified) {
            ...
            let reason: IDeviceStageAuthFailureReasonValue = 'unofficialDevice';
          ...
          if (err?.className === EOneKeyErrorClassNames.OneKeyServerApiError) {
            reason = 'unknown';
          } else if (
            ...
            reason = 'defective';
          } else if (err?.code === HardwareErrorCode.NotAllowInBootloaderMode) {
      ...
      for (;;) {
        ...
        >((resolve) => {
          const onAction = ({
            ...
            appEventBus.off(EAppEventBusNames.DeviceStageAuthAction, onAction);
            resolve(next);
          };
