...
export function SetupStatusCard({ tone, label, ...rest }: ISetupStatusCardProps) {
  return (
    <SetupCard
      elevated
      {...rest}
      backgroundSlot={<SetupCardBackground variant={tone === "ready" ? "brand" : "neutral"} />}
    >
      <SetupCardBody alignItems="center">
        <YStack h={240} pt="$6" gap="$5" alignItems="center" justifyContent="center">
          {tone === "checking" ? (
            <Spinner size="large" />
          ) : (
