...
import type { IHomeHistoryLegacyPayload } from '../sections/history/homeHistorySourceAdapter';
...
type IHomeLegacyHistorySectionState =
  ...
  | {
      kind: 'ready';
      viewState: 'ready';
      payload: IHomeHistoryLegacyPayload;
      freshness: 'live' | 'confirmedCache';
      refresh: 'idle' | 'refreshing' | 'failed';
    };

type IHomeLegacyHistorySemanticState =
  | { kind: 'loading'; viewState: 'loading' }
  | { kind: 'empty'; viewState: 'empty' }
  | { kind: 'error'; viewState: 'empty'; refresh: 'failed' }
  | {
...
function adaptHomeLegacyHistorySemanticState({
  semantic,
}: {
  semantic?: IHomeSectionSemanticModel;
}): IHomeLegacyHistorySemanticState | undefined {
  if (!semantic) {
    return undefined;
  }
...
export { adaptHomeLegacyHistorySection, adaptHomeLegacyHistorySemanticState };
export type { IHomeLegacyHistorySectionState, IHomeLegacyHistorySemanticState };
