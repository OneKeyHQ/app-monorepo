...
type IHomeLegacyDeFiSectionState =
  ...
  | {
      kind: 'ready';
      viewState: 'ready';
      payload: IHomeDeFiLegacyPayload;
      freshness: 'live' | 'confirmedCache';
      refresh: 'idle' | 'refreshing' | 'failed';
    };

type IHomeLegacyDeFiSemanticState =
  | { kind: 'loading'; viewState: 'loading' }
  | { kind: 'empty'; viewState: 'empty' }
  | { kind: 'error'; viewState: 'empty'; refresh: 'failed' }
  | {
...
function adaptHomeLegacyDeFiSemanticState({
  semantic,
}: {
  semantic?: IHomeSectionSemanticModel;
}): IHomeLegacyDeFiSemanticState | undefined {
  if (!semantic) {
    return undefined;
  }
...
export { adaptHomeLegacyDeFiSection, adaptHomeLegacyDeFiSemanticState };
export type { IHomeLegacyDeFiSectionState, IHomeLegacyDeFiSemanticState };
