...
function removeFeeInfoInTx(encodedTx: IEncodedTxEvm) {
  ...
  // DELETE price and use wallet re-calculated fee price
...
const TransactionConfirm = () => {
  ...
  const [encodedTx, setEncodedTx] = useState<IEncodedTx | null>(
    isFromDapp
  ...
  useEffect(() => {
    setEncodedTx(params.encodedTx ?? null);
  }, [params.encodedTx]);
  ...
  const { decodedTxLegacy: decodedTx } = useDecodedTx({ encodedTx, payload });
