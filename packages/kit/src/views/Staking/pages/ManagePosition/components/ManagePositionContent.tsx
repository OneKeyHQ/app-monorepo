...
export function ManagePositionContent({
  ...
  const retryFooterAction = useMemo<IManagePositionFooterAction | undefined>(
    () =>
      hasLoadError
        ? {
  ...
  // then the switch-loading state (if any).
  const footerAction =
    noConnectedWalletFooterAction ?? retryFooterAction ?? switchingFooterAction;
