...
describe('navigateToMarketTokenDetail', () => {
  ...
  it('clears stale detail for an unresolved non-stock token without a preview', async () => {
    await navigateToMarketTokenDetail(
      { address: '', networkId: 'evm--1', isNative: true },
      { tokenDetailActions, resolveMarketAsset: true },
    );
    jest.runAllTimers();
