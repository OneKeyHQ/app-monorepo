...
export function formatTokenPrice(price: string | number, priceDecimals: number): string {
...
export function formatTokenAmount(amount: string | number, weiDecimals: number, szDecimals: number): string {
...
export function calculateSlippagePrice(
  ...
  const priceWithSlippage = price.multipliedBy(1 + slippageDirection * slippage);
  const priceDecimals = getPriceDecimals(szDecimals);
  return formatTokenPrice(priceWithSlippage.toFixed(), priceDecimals);
}

export function validatePricePrecision(price: string | number, szDecimals: number): boolean {
...
export function getAmountLotSize(weiDecimals: number, szDecimals: number): string {
  const amountDecimals = weiDecimals - szDecimals;
  return new BigNumber(10).exponentiatedBy(-amountDecimals).toFixed();
}

export function displayPrice(price: string | number, szDecimals: number): string {
  const priceDecimals = getPriceDecimals(szDecimals);
...
export function formatPriceToSignificantDigits(price: number, maxDigits: number = 5): string {
