...
const ABSTRACTION_MODE_OPTIONS = [
  { label: 'Unified Account', value: 'u' as const },
  { label: 'Disabled', value: 'i' as const },
  { label: 'Portfolio Margin', value: 'p' as const },
];
...
function DevAbstractionModeSelector() {
  ...
  const handleSetMode = async (mode: 'i' | 'u' | 'p') => {
    ...
    try {
      ...
      await backgroundApiProxy.serviceHyperliquid.fetchUserAbstraction(
        activeAccount.accountAddress as `0x${string}`,
      );
    } catch (e) {
  ...
  return (
    <YStack px="$2.5" pt="$2" gap="$2" borderTopWidth={1} borderColor="$borderSubdued" mt="$2">
