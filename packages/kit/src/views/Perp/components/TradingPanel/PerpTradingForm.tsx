...
import { validatePriceInput, formatPriceToSignificantDigits, formatPercentage } from '../../utils/tokenUtils';
...
function PerpTradingForm({ isSubmitting = false }: IPerpTradingFormProps) {
    ...
    const updateForm = useCallback((updates: Partial<ITradingFormData>) => {
    ...
    useEffect(() => {
        ...
        if (prevType !== 'limit' && currentType === 'limit' && !formData.price && tokenInfo?.markPx) {
    ...
    const calculateTpPrice = useCallback((gainPercent: string) => {
    ...
    const calculateSlPrice = useCallback((lossPercent: string) => {
    ...
    const calculateTpPercent = useCallback((tpPrice: string) => {
        if (!tpPrice || !referencePrice) return '';
        const tp = parseFloat(tpPrice);
        const diff = formData.side === 'long' ? (tp - referencePrice) : (referencePrice - tp);
        const percent = (diff / referencePrice) * 100;
        return formatPercentage(percent);
    }, [referencePrice, formData.side]);

    const calculateSlPercent = useCallback((slPrice: string) => {
        if (!slPrice || !referencePrice) return '';
        const sl = parseFloat(slPrice);
        const diff = formData.side === 'long' ? (referencePrice - sl) : (sl - referencePrice);
        const percent = (diff / referencePrice) * 100;
        return formatPercentage(percent);
    }, [referencePrice, formData.side]);
        ...
        <>
            <YStack gap="$4" h={400}>
                ...
                <YStack gap="$3">
                    ...
                    <Checkbox
                        ...
                        onChange={(checked: ICheckedState) => updateForm({ hasTpsl: !!checked })}
                        disabled={isSubmitting}
            ...
            <YStack bg="$bgSubdued" borderRadius="$3" borderWidth="$0" p="$3">
                <XStack justifyContent="space-between">
                    ...
                    >
                        {formatPriceToSignificantDigits(Number(formData.size) * Number(referencePrice), 2)}
                    </NumberSizeableText>
                </XStack>
                <XStack justifyContent="space-between">
                    <SizableText size="$bodyMd" color="$textSubdued">
                    ...
                    {leverage ? <NumberSizeableText
                        size="$bodyMd"
                        formatter="value"
                        formatterOptions={{ currency: '$' }}
                    >
                        {formatPriceToSignificantDigits(Number(formData.size) * Number(referencePrice) / leverage, 2)}
                    </NumberSizeableText> : <Skeleton width={80} height={18} />}
                </XStack>
            </YStack>
        </>
    );
