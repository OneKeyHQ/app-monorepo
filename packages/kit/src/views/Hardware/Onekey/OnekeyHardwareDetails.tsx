        ...
        {!!canOnDeviceInputPin && (
          ...
          >
            <Switch
              ...
              onToggle={() => {
                // classic & mini firmware version > 2.3.0
                const version =
                  getDeviceFirmwareVersion(deviceFeatures).join('.');
                if (semver.valid(version) && semver.lt(version, '2.3.0')) {
                  ToastManager.show(
                    {
                      title: intl.formatMessage({
                        id: 'msg__upgrade_firmware_to_enable',
                      }),
                    },
                    {
                      type: 'error',
