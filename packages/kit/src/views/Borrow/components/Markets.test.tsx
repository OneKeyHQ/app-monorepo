...
describe('Markets selection intent', () => {
  ...
  it('cancels pending B when the user chooses published A again', () => {
  ...
  it('replaces pending B with the latest requested C', () => {
    setContext(markets[1]);
    const screen = render(<Markets />);
  ...
  it('uses the latest published market when an open Select retains its callback', () => {
    setContext(markets[1]);
    const screen = render(<Markets />);
  ...
  it('cancels a newer request even when selection was deferred from an idle render', () => {
    const screen = render(<Markets />);
    const retainedOnChange = screen.UNSAFE_root.findByProps({
      testID: BorrowTestIDs.marketSelect,
    }).props.onChange as (value: string) => void;
