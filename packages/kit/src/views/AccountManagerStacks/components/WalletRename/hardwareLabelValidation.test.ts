...
const validatePro2Label = (value: string) =>
  getHardwareLabelValidationError({
    value,
    maxLength: 32,
    asciiAlphanumericWithSpacesOnly: true,
  });

describe('getHardwareLabelValidationError', () => {
  test.each(['OneKeyPro2', 'OneKey Pro 2', 'ONEKEY', '123456'])(
    'accepts a supported Pro2 label: %s',
    (value) => {
  ...
  test.each([
    ...
    'OneKey_Pro2',
    'OneKey　Pro2',
  ])('rejects an unsupported Pro2 label: %s', (value) => {
    expect(validatePro2Label(value)).toBe('invalid');
  });

  it('reports an overlong supported label', () => {
    expect(validatePro2Label('A'.repeat(33))).toBe('tooLong');
  });
