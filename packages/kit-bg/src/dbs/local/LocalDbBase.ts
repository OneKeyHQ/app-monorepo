...
const SECRET_ENCRYPT_V2_MAGIC_HEX = Buffer.from('1K_ENC_V2', 'utf8').toString(
  'hex',
);
...
export abstract class LocalDbBase extends LocalDbBaseContainer {
  ...
  async isLocalPasswordKdfLazyUpgradeCompleted(): Promise<boolean> {
    const ctx = await this.getContext();
    return Boolean(ctx.localPasswordKdfUpgraded);
  }
  ...
  isLocalPasswordKdfCredentialUpgradeCandidate({
    ...
    return !stripLocalSecretPrefix(credential.credential).startsWith(
      SECRET_ENCRYPT_V2_MAGIC_HEX,
    );
  }
