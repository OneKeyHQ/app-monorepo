...
describe('LocalDbBase local secret envelope credentials', () => {
  ...
  it('best-effort deletes skipped imported credential layer keys during restore', async () => {
    const db = new TestLocalDb();
    const password = await encodePasswordAsync({ password: 'test-password' });
    db.context.localPasswordKdfUpgraded = true;
    db.context.localPasswordKdfUpgradedTargetIterations =
    ...
    const concurrentImportedCredential = await encryptImportedCredential({
      credential: { privateKey: 'concurrent-private-key-hex' },
      password,
    });
    const dbName = `test-lse-imported-skip-cleanup-${Math.random()}`;
    ...
    const txAddRecords = db.txAddRecords.bind(db);
    db.txAddRecords = jest.fn(
      async <T extends ELocalDBStoreNames>(
        params: ILocalDBTxAddRecordsParams<T>,
      ) => {
        if (
          params.name === ELocalDBStoreNames.Credential &&
          db.credentials.length === 0
        ) {
          db.credentials = [
            {
              id: accountId,
              credential: concurrentImportedCredential,
            },
          ];
        }
        return txAddRecords(params);
      },
    ) as TestLocalDb['txAddRecords'];
