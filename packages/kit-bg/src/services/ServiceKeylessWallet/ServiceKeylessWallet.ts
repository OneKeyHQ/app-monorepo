...
class ServiceKeylessWallet extends ServiceBase {
  ...
  async apiUploadKeylessBackendShare(params: {
    ...
    const ownerId = `${provider}:${socialAccountId}`;
    return {
      ownerId,
      encryptedMnemonic,
      backendShare,
  ...
  @backgroundMethod()
  @toastIfError()
  async createKeylessWalletV2(params: { token: string }) {
    const { token } = params;
    ...
    const juiceboxShareData: { success: boolean } =
      await this.apiUploadKeylessJuiceboxShare({
        token,
        juiceboxShare,
      });
    return this.backgroundApi.serviceAccount.createHDWallet({
      mnemonic,
