...
class ServiceKeylessWallet extends ServiceBase {
  ...
  private static readonly MOCKED_KEYLESS_SHARES_STORAGE_KEY =
    'keyless_mocked_shares';

  private async getMockedKeylessShares(): Promise<{
    [ownerId: string]: {
      backendShare: IKeylessBackendShare | undefined;
      juiceboxShare: IKeylessJuiceboxShare | undefined;
    };
  }> {
    const data = await appStorage.getItem(
      ServiceKeylessWallet.MOCKED_KEYLESS_SHARES_STORAGE_KEY,
    );
    if (!data) {
      return {};
    }
  ...
  private async saveMockedKeylessShares(shares: {
    [ownerId: string]: {
      backendShare: IKeylessBackendShare | undefined;
      juiceboxShare: IKeylessJuiceboxShare | undefined;
    };
  ...
  @backgroundMethod()
  @toastIfError()
  async clearMockedKeylessShares(): Promise<void> {
  ...
  async apiGetKeylessBackendShare(params: {
    ...
    const { token } = params;
    // verify token by supabase SDK, make sure token is valid and generated by Google or Apple
    // decode token to get social account id
    // hash social account id by env salt and KMS
  ...
  async apiUploadKeylessBackendShare(params: {
    ...
    mockedShares[ownerId].backendShare = backendShareData;
