...
export type IFirmwareArtifactSelfTestDescriptor = {
  ...
  label: string;
  version: string;
  role: IFirmwareDownloadArtifact['role'];
  container: IFirmwareDownloadArtifact['container'];
  expectedSize?: number;
};
...
const getProRelease = async () => {
  const config = await getFirmwareManifestSnapshot({
    preRelease: false,
    forceRefresh: true,
  });
  const release = config.pro['firmware-v8']?.[0];
  if (!release) {
    throw new OneKeyLocalError('Remote Pro firmware is unavailable');
  }
  return release;
};
...
export const getFirmwareArtifactSelfTestArtifact = async (
  ...
  descriptor: IFirmwareArtifactSelfTestDescriptor;
  artifact: IFirmwareDownloadArtifact;
  ...
  if (scenario === 'pro-firmware') {
    ...
    expectedSha256 = release.fingerprint;
    label = 'Pro firmware';
    artifactId = 'firmware';
  } else if (scenario === 'pro-resource') {
    url = release.resource;
    expectedSize = release.resourceExpectedSize;
    expectedSha256 = release.resourceFingerprint;
    label = 'Pro incremental resource';
    artifactId = 'resource';
  } else {
    url = release.fullResource;
    expectedSize = release.fullResourceExpectedSize;
    expectedSha256 = release.fullResourceFingerprint;
  ...
  const artifact = resolveFirmwarePlanArtifact({
    ...
    target: artifactId,
    url,
    container: artifactId === 'resource' ? 'zip' : 'raw',
    expectedSize,
    expectedSha256,
  });
...
const readPreparedArtifact = async ({
  ...
  if (
    reader.size !== artifact.size ||
    (expectedSize !== undefined && artifact.size !== expectedSize)
  ) {
  ...
  try {
    while (bytesRead < totalBytes) {
      ...
      chunkCount += 1;
      onProgress?.(bytesRead, chunkCount, reader.size);
    }
  } finally {
    await prepared.artifactReader.close({ readerId: reader.readerId });
  }
...
export const executeFirmwareArtifactSelfTest = async ({
  ...
  const { artifact, artifactId, descriptor } =
  ...
  const mainResult = await controller.withPreparedPlanArtifacts(
    ...
    async (prepared) => {
      ...
      const readerResult = await readPreparedArtifact({
        ...
        artifactId,
        expectedSize: artifact.expectedSize,
        onProgress: (bytesRead, chunkCount, totalBytes) => {
          onProgress({
            phase: 'reading',
            progress: 65 + Math.floor((bytesRead / totalBytes) * 20),
            bytesRead,
            chunkCount,
          });
        },
