...
export const downloadFirmwareArtifact = async ({
  artifact,
  artifactId,
  taskId,
  transactionId,
  ...
  trackArtifactDownload(transactionId, async () => {
    ...
    try {
      ...
      firmwareUpdateTrace({
        transactionId,
        stage: 'artifact-download-complete',
        artifactId,
        artifactRole: artifact.role,
        expectedBytes: artifact.expectedSize,
      ...
      firmwareUpdateTrace({
        ...
        durationMs: Date.now() - startedAt,
        errorCode:
          errorMessage.match(/ARTIFACT_[A-Z0-9_]+/)?.[0] ??
  ...
  try {
    ...
    for (const [index, planArtifact] of planArtifacts.entries()) {
      ...
      const receipt = await downloadFirmwareArtifact({
        ...
        taskId: `fw-${plan.planDigest.slice(0, 24)}-${index}`,
        transactionId,
        leaseRef,
        deadlineAt,
      });
