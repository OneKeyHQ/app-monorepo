...
class ServiceFirmwareUpdate extends ServiceBase {
  ...
  @backgroundMethodForDev()
  getFirmwareArtifactSelfTestState(): Promise<
  ...
  async checkAllFirmwareRelease({
    ...
    const firmware = await this.checkFirmwareRelease({
      ...
      forceUpdate: forceUpdateTargets.includes('firmware'),
    });
    ...
    if (
      ...
      firmware?.toVersion &&
      (firmware.hasUpgrade || forceUpdateTargets.includes('bootloader'))
    ) {
      bootloader = await this.checkBootloaderRelease({
        ...
        forceUpdate: forceUpdateTargets.includes('bootloader'),
      });
    }
    ...
    if (!bridge?.shouldUpdate) {
      ble = await this.checkBLEFirmwareRelease({
        ...
        forceUpdate: forceUpdateTargets.includes('ble'),
      });
    }
  ...
  baseCheckAllFirmwareRelease(
    ...
    return this.loadBaseFirmwareRelease(params).then((result) => ({
      ...result,
    ...
    return this.withFirmwareUpdateEvents(async () => {
      ...
      try {
        ...
        const updateResult = await convertDeviceResponse(async () => {
          ...
          return executePreparedFirmwareUpdateV3({
            sdk: hardwareSDK,
            connectId: updatingConnectId,
            ...executionArtifacts,
            platform: platformEnv.symbol ?? 'web',
