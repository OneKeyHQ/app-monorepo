...
import type { IPrimeConfigFlushInfo } from '@onekeyhq/shared/types/socket';
...
class ServicePrimeCloudSync extends ServiceBase {
  ...
  async buildFlushLock({
    ...
    const flushLock = await this.syncManagers.lock.buildSyncItem({
      syncCredential,
      target: this.syncManagers.lock.staticSyncTarget,
      dataTime: await this.timeNow(),
    });
    if (!flushLock?.data) {
      throw new Error('flushLock.data is not found');
    }
    return flushLock;
  }
  ...
  async apiUploadItems({
    ...
    const { masterPasswordUUID } = await primeMasterPasswordPersistAtom.get();
    let pwdHash = '';
    let flushLock: IDBCloudSyncItem | undefined;
    ...
    if (isReset) {
      ...
      if (isFlush) {
        ...
        flushLock = await this.buildFlushLock({
          syncCredential,
        });
      }
    }
  ...
  _callApiUploadItems = async ({
    ...
    flushLock: IDBCloudSyncItem | undefined;
    pwdHash: string;
  ...
  async onWebSocketConfigFlush(payload: IPrimeConfigFlushInfo) {
    const { masterPasswordUUID } = await primeMasterPasswordPersistAtom.get();
  ...
  async initLocalSyncItemsDB({
    ...
    await this.backgroundApi.serviceAccount.generateMissingHDWalletHash({
      password,
    });
  ...
  async enableCloudSync(): Promise<{
      ...
      await this.withDialogLoading(
        ...
        async () => {
          ...
          if (isServerMasterPasswordSet) {
            ...
            await this.backgroundApi.serviceMasterPassword.verifyServerMasterPassword(
              {
                syncCredential: credential,
                clearLocalMasterPassword: true,
              },
            );
          }
          return credential;
    ...
    return { success: true, isServerMasterPasswordSet };
  }
