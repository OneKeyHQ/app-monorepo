...
> {
  ...
  override async buildSyncRawKey({
    ...
    if (!walletXfp) {
      throw new OneKeyPlainTextError('buildWalletSyncKey ERROR: walletXfp is required');
    }
    if (isNil(accountIndex)) {
      throw new OneKeyPlainTextError('buildWalletSyncKey ERROR: accountIndex is required');
    }
  ...
  override async buildSyncPayload({
    ...
    if (!walletXfp) {
      throw new OneKeyPlainTextError('buildSyncPayload ERROR: walletXfp is required');
    }
    if (isNil(indexedAccount.index)) {
      throw new OneKeyPlainTextError('buildSyncPayload ERROR: accountIndex is required');
    }
