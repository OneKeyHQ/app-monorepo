...
describe('ServiceThirdPartyHardware Trezor BLE binding', () => {
  ...
  it('matches the Trezor DB device before persisting bleConnectId', async () => {
    ...
    // Credentials minted during binding must drain before the finally-block
    // disconnect.
  ...
  // Unreadable identity = "could not verify", never a mismatch verdict.
  ...
  // A pairing request no longer counts as "not this device"; a user-aborted
  // pairing surfaces as a real failure, not a silent null.
