...
describe('ServiceHardware.connect WebUSB reuse', () => {
  ...
  it('桌面 BLE 搜索结果直接使用 Noble peripheral id，不替换成 USB 序列号', async () => {
    ...
    await expect(
      service.getVerifiedBleConnectIdForPersistence({
        connectId: blePeripheralId,
      }),
    ).resolves.toBe(blePeripheralId);
  });

  it('does not record a BLE endpoint when the live connection fails', async () => {
    mutablePlatformEnv.isSupportDesktopBle = true;
    const service = new ServiceHardware({
      backgroundApi: {} as IBackgroundApi,
    });
    ...
    jest
      .spyOn(service, 'connectDevice')
      .mockRejectedValue(new Error('BLE connect failed'));
    ...
    ).rejects.toThrow('BLE connect failed');
    await expect(
      service.getVerifiedBleConnectIdForPersistence({
        connectId: blePeripheralId,
      }),
  ...
  it('live-connects and records a forced BLE endpoint when commType is missing', async () => {
    mockedHardwareForceTransportAtomGet.mockResolvedValue({
    ...
    jest
      .spyOn(service, 'getCompatibleConnectId')
      .mockResolvedValue(blePeripheralId);
    const connectDevice = jest
      .spyOn(service, 'connectDevice')
    ...
    await service.connect({
      device: {
        ...
        deviceType: 'pro2',
        name: 'Pro 2 0088',
        features: { deviceId: 'STALE_DEVICE_ID' },
      } as unknown as SearchDevice,
    });
  ...
  it('clears an older BLE endpoint after the same device live-connects over USB', async () => {
    mockedHardwareForceTransportAtomGet.mockResolvedValue({
      forceTransportType: EHardwareTransportType.DesktopWebBle,
    });
    const service = new ServiceHardware({
    ...
    jest
      .spyOn(service, 'getCompatibleConnectId')
      .mockResolvedValueOnce(blePeripheralId)
      .mockResolvedValueOnce('USB_SERIAL');
    jest
      .spyOn(service, 'connectDevice')
      .mockResolvedValue({ deviceId: 'PRO2_DEVICE_ID' } as Features);
    ...
    await service.connect({
      device: buildDevice({ deviceType: 'pro2' }),
    });
