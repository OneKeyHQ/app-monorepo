...
export const getFinalizedBlockInfo = memoizee(
  async (
    networkId: string,
    backgroundApi: IBackgroundApi,
    apiPromise?: ApiPromise,
    ...
    try {
      if (apiPromise) {
        const res = await apiPromise.rpc.chain.getFinalizedHead();
        const block = await apiPromise.rpc.chain.getBlock(res.toHex());
        return {
          blockHash: res.toHex(),
          blockNumber: block.block.header.number.toNumber(),
      ...
      return getBlockInfo(networkId, backgroundApi, apiPromise);
    }
  },
  {
    maxAge: timerUtils.getTimeDurationMs({ seconds: 10 }),
