...
type ITokenValue = string;
type IToken = [ITokenType, ITokenValue];

// Returns Token and Token byte size
function nextToken(tagStr: string, pos: number): [IToken, number] {
  const c = tagStr[pos];
  if (c === ':') {
...
function tokenize(tagStr: string): IToken[] {
  let pos = 0;
  const tokens = [];
