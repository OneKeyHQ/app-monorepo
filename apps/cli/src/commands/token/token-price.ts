...
export function registerTokenPriceCommand(parent: Command): void {
  parent
    .command('price')
    .description('Get token price and price changes')
    ...
    .action(async (options: { chain: string; token: string }, command) => {
      ...
      try {
        ...
        assertChainCapability(chainConfig, 'evmTokenMarket', 'token-price');
