...
const pbxprojPath = path.join(rootDir, 'ios/OneKeyWallet.xcodeproj/project.pbxproj');

const envVariant = (process.env.ONEKEY_BUILD_VARIANT || '').toLowerCase();
const allowSkipGpg = (process.env.ONEKEY_ALLOW_SKIP_GPG_VERIFICATION || '').toLowerCase() === 'true';
const variant = envVariant === 'skip_gpg' || allowSkipGpg ? 'skip_gpg' : 'standard';
...
function updateInfoPlistDisplayName(filePath, displayName) {
  ...
  const pattern = /(<key>CFBundleDisplayName<\/key>\s*<string>)([^<]*)(<\/string>)/;
