...
class BackgroundReactNativeDelegate: ExpoReactNativeFactoryDelegate {
  ...
  static let backgroundBundleDebugURL = "http://localhost:8082/apps/mobile/background.bundle?platform=ios&dev=true&lazy=false&minify=false&inlineSourceMap=false&modulesOnly=false&runModule=true&excludeSource=true&sourcePaths=url-server&app=so.onekey.wallet&transform.routerRoot=app&transform.engine=hermes&transform.bytecode=1&unstable_transformProfile=hermes-stable"
  ...
  override func bundleURL() -> URL? {
    ...
    return RCTBundleURLProvider.sharedSettings().jsBundleURL(BackgroundReactNativeDelegate.backgroundBundleDebugURL)
#else
