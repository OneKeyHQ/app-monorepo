...
import type { TitleBarOptions } from './options';
...
export class CustomTitlebar extends ThemeBar {
  ...
  private setupBackgroundColor() {
    ...
    if (!color) {
      const metaColor =
        ...
        document.querySelectorAll('meta[name="msapplication-TileColor"]');
  ...
  private setupMenubar() {
    ipcRenderer
      .invoke('request-application-menu')
  ...
  private loadEvents() {
    ...
    this.onDidChangeMaximized(
      ipcRenderer.sendSync('window-event', 'window-is-maximized'),
    );

    ipcRenderer.on('window-maximize', (_, isMaximized) =>
      this.onDidChangeMaximized(isMaximized),
    );
    ipcRenderer.on('window-fullscreen', (_, isFullScreen) =>
      this.onWindowFullScreen(isFullScreen),
    );
    ipcRenderer.on('window-focus', (_, isFocused) =>
  ...
  private onDidChangeMaximized(isMaximized: boolean) {
    ...
    if (maximize) {
      maximize.title = isMaximized
        ? this.currentOptions.tooltips?.restoreDown!
        : this.currentOptions.tooltips?.maximize!;
  ...
  private updateStyles() {
    ...
    const updatedWindowControls = ipcRenderer.sendSync(
