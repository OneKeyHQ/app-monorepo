...
export function KeyChord(firstPart: number, secondPart: number): number {
  const chordPart = ((secondPart & 0x00_00_ff_ff) << 16) >>> 0;
...
export function createKeybinding(
  ...
  const firstPart = (keybinding & 0x00_00_ff_ff) >>> 0;
  const chordPart = (keybinding & 0xff_ff_00_00) >>> 16;
  if (chordPart !== 0) {
