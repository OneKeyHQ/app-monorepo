...
import type { IKeyboardEvent } from '../browser/keyboardEvent';
import { StandardKeyboardEvent } from '../browser/keyboardEvent';
import type { IMouseEvent } from '../browser/mouseEvent';
...
import { Disposable, dispose, toDisposable } from './lifecycle';
...
const _manualClassList = new (class implements IDomClassList {
  ...
  addClasses(node: HTMLElement, ...classNames: string[]): void {
    classNames.forEach((nameValue) =>
      nameValue.split(' ').forEach((name) => this.addClass(node, name)),
    );
  }
  ...
  removeClass(node: HTMLElement, className: string): void {
    ...
    if (this._lastStart === -1) {
      ...
      node.className =
        ...
        node.className.substring(this._lastEnd || 0);
    }
  }
  ...
  toggleClass(
    ...
    if (
      this._lastStart !== -1 &&
      (shouldHaveIt === undefined || !shouldHaveIt)
    ) {
...
const _nativeClassList = new (class implements IDomClassList {
  hasClass(node: HTMLElement, className: string): boolean {
    return (
      Boolean(className) && node.classList && node.classList.contains(className)
    );
  }
...
const _classList: IDomClassList = browser.isIE
  ? _manualClassList
  : _nativeClassList;
export const hasClass: (node: HTMLElement, className: string) => boolean =
  _classList.hasClass.bind(_classList);
export const addClass: (node: HTMLElement, className: string) => void =
  _classList.addClass.bind(_classList);
export const addClasses: (node: HTMLElement, ...classNames: string[]) => void =
  _classList.addClasses.bind(_classList);
export const removeClass: (node: HTMLElement, className: string) => void =
  _classList.removeClass.bind(_classList);
export const removeClasses: (
  node: HTMLElement,
  ...classNames: string[]
) => void = _classList.removeClasses.bind(_classList);
...
export function measure(callback: () => void): IDisposable {
  return scheduleAtNextAnimationFrame(callback, 10_000 /* must be early */);
}

export function modify(callback: () => void): IDisposable {
  return scheduleAtNextAnimationFrame(callback, -10_000 /* must be late */);
}
...
const DEFAULT_EVENT_MERGER: any = function (
  lastEvent: DOMEvent,
...
const convertToPixels: (element: HTMLElement, value: string) => number =
  (function () {
