...
export class CETSubMenu extends CETMenuItem {
  ...
  constructor(
    item: MenuItem,
    private submenuIcons: IMenuIcons,
    private submenuItems: MenuItem[],
    private parentData: ISubMenuData,
    private submenuParentOptions: MenuBarOptions,
    ...
    this.hideScheduler = new RunOnceScheduler(() => {
      if (
        this.element &&
        !isAncestor(document.activeElement, this.element) &&
  ...
  private createSubmenu(selectFirstItem = true): void {
    ...
    if (this.element) {
      if (!this.parentData.submenu) {
        ...
        this.parentData.submenu = new CETMenu(
          this.submenuContainer,
          this.submenuIcons,
          this.submenuParentOptions,
          this.submenuOptions,
          this._closeSubMenu,
        );
        ...
        const computedStyles = getComputedStyle(
          this.parentData.parent.container,
        );
        ...
        if (window.innerWidth <= boundingRect.right + childBoundingRect.width) {
          ...
          this.submenuContainer.style.top = `${
            this.element.offsetTop + boundingRect.height
          }px`;
        } else {
          this.submenuContainer.style.left = `${this.element.offsetWidth}px`;
          this.submenuContainer.style.top = `${
            this.element.offsetTop - paddingTop
          }px`;
        }

        this.submenuDisposables.push(
          addDisposableListener(
            this.submenuContainer,
            EventType.KEY_UP,
            (e) => {
  ...
  protected override applyStyle(): void {
    ...
    applyFill(
      this.submenuIndicator,
      this.submenuParentOptions.svgColor,
      fgColor,
    );
