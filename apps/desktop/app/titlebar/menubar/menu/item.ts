...
import type { KeyCode } from '../../base/common/keyCodes';
...
export class CETMenuItem extends Disposable implements IMenuItem {
  ...
  constructor(
    ...
    if (this._item.label && options.enableMnemonics) {
      ...
      if (label) {
        ...
        if (matches) {
          this._mnemonic = KeyCodeUtils.fromString(
  ...
  render(el: HTMLElement): void {
    ...
    this._register(
      addDisposableListener(this.element!, EventType.MOUSE_DOWN, (e) => {
    ...
    this._register(
      addDisposableListener(this.element!, EventType.CLICK, (e) => {
    ...
    if (this.itemElement) {
      ...
      if (this.iconElement) {
        if (this.iconElement.firstElementChild?.className === 'icon') {
          applyFill(
            this.iconElement.firstElementChild as HTMLElement,
            this.parentOptions?.svgColor,
            fgColor,
            false,
          );
        } else {
  ...
  setAccelerator(): void {
    ...
    if (this.itemElement && accelerator !== null) {
      append(this.itemElement, $('span.keybinding')).textContent =
  ...
  updateIcon(): void {
    if (this.item.icon) {
      ...
      this.iconElement!.innerHTML = this.item.checked
        ? this.menuIcons.radioChecked
        : this.menuIcons.radioUnchecked;
    }
