...
export const trezorBleFlags = {
   ...
   * proxy blinding the scan. Tied to the app's own dev/packaged distinction
   * (same as BlePair.ts/BaseProcess.ts) instead of its own env var — it is
   * purely diagnostic (see maybeRawWatch), so losing it in a packaged build is
   * a non-issue.
   */
  rawWatch: isDev,
};
...
export function logTrezorBleFlags(): void {
  logger.info(
    `[TrezorBLE] flags ${JSON.stringify(trezorBleFlags)} ` +
      `(override with ONEKEY_BLE_PAIR_KEEP_LINK = 0|1)`,
  );
}
