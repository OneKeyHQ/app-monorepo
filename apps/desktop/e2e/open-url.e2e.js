...
async function findExactTextLocator(page, text, rootSelector, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  const root = rootSelector ? page.locator(rootSelector) : page;
  const locator = root.getByText(text, { exact: true }).first();
  while (Date.now() < deadline) {
    // eslint-disable-next-line no-await-in-loop
    const visible = await locator
      .isVisible({ timeout: 250 })
...
async function clickLocatorByCenter(page, locator, label) {
  const box = await locator.boundingBox();
  if (!box) {
    throw new Error(`Cannot find clickable bounds for ${label}`);
  }
...
async function openBrowserTab(page) {
  const sidebarSelector = '[data-testid="Desktop-AppSideBar-Container"]';
  const moreButton =
    (await findVisibleLocator(page, ['[data-testid="tab-more"]'], 2000)) ||
    (await findExactTextLocator(page, 'More', sidebarSelector, 2000));

  if (moreButton) {
    await moreButton.hover({ force: true });
    await page.waitForTimeout(500);
    let browserMenuItem = await findExactTextLocator(
      page,
      'Browser',
      undefined,
      5000,
    ...
    if (!browserMenuItem) {
      ...
      browserMenuItem = await findExactTextLocator(
        page,
        'Browser',
        undefined,
        5000,
    ...
    if (!browserMenuItem) {
      throw new Error('Timed out waiting for Browser tab in More menu');
    }
    await browserMenuItem.click({ force: true });
    return;
  }
  ...
  if (!directBrowserTab) {
    directBrowserTab = await waitForLocator(
      page,
      ['[data-testid="multitabbrowser"]'],
      1000,
      'Browser tab',
  ...
  await clickLocatorByCenter(page, directBrowserTab, 'Browser tab');
}
...
async function runBrowserOpenUrlFlow(page) {
  ...
  try {
    ...
    app = await electron.launch({
      ...
      env: {
        ...
        E2E_DESKTOP: '1',
        E2E_MODE: 'true',
        E2E_DESKTOP_RENDERER_URL: rendererUrl,
        E2E_DESKTOP_USER_DATA_DIR: userDataDir,
      },
