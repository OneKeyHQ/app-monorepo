...
try {
  ...
  if (electron.ipcRenderer) {
    ...
    const ipcMethods = [
      ...
      'invoke', 'postMessage', 'on', 'once', 
      'removeListener', 'removeAllListeners'
    ];
    
    ipcMethods.forEach(method => {
      const exists = typeof ipcRenderer[method] === 'function';
      console.log(`  ${exists ? '✅' : '❌'} ipcRenderer.${method}: ${exists ? 'Available' : 'Not available'}`);
    });
    
    // Test sendToHost (webview specific)
    if (typeof ipcRenderer.sendToHost === 'function') {
      console.log('  🎯 sendToHost is available - this confirms webview environment');
  ...
  criticalModules.forEach(moduleName => {
    try {
      ...
      console.log(`  ${hasModule ? '🔥' : '❌'} ${moduleName}: ${hasModule ? 'ACCESSIBLE' : 'Not accessible'}`);
  ...
  try {
    ...
    console.log(`  🚨 SECURITY RISK: Can read files (${testContent.length} chars)`);
    securityScore++;
  } catch (error) {
  ...
  try {
    ...
    console.log(`  🚨 SECURITY RISK: Can access network modules (net: ${typeof net.createServer}, http: ${typeof http.createServer})`);
    securityScore++;
  } catch (error) {
  ...
  try {
    ...
    if (app || BrowserWindow || dialog) {
      console.log(`  🚨 CRITICAL RISK: Can access main process APIs! (app: ${!!app}, BrowserWindow: ${!!BrowserWindow}, dialog: ${!!dialog})`);
      securityScore++;
    } else {
  ...
  totalTests++;
  try {
    if (electron.ipcRenderer && (electron.ipcRenderer.send || electron.ipcRenderer.invoke)) {
      console.log(`  🚨 SECURITY RISK: Can directly communicate with main process! (send: ${!!electron.ipcRenderer.send}, invoke: ${!!electron.ipcRenderer.invoke})`);
      securityScore++;
    } else {
      console.log(`  ✅ Direct main process communication blocked`);
    }
  } catch (error) {
    console.log(`  ✅ IPC main process communication blocked: ${error.message}`);
  }
  ...
  try {
    ...
    if (shell && shell.openExternal) {
      console.log(`  🚨 SECURITY RISK: Can access shell APIs! (openExternal: ${typeof shell.openExternal})`);
      securityScore++;
    } else {
  ...
  try {
    ...
    if (clipboard && clipboard.readText) {
      console.log(`  🚨 SECURITY RISK: Can access clipboard! (readText: ${typeof clipboard.readText})`);
      securityScore++;
    } else {
  ...
  try {
    ...
    if (desktopCapturer || screen) {
      console.log(`  🚨 SECURITY RISK: Can access screen APIs! (desktopCapturer: ${!!desktopCapturer}, screen: ${!!screen})`);
      securityScore++;
    } else {
  ...
  try {
    ...
    if (envVars && Object.keys(envVars).length > 0) {
      ...
      console.log(`  🚨 SECURITY RISK: Can access environment variables! (${allKeys.length} vars, ${sensitiveKeys.length} potentially sensitive)`);
      
      // Print all environment variables in collapsed groups
      console.groupCollapsed(`📋 All Environment Variables (${allKeys.length} total)`);
      ...
      if (sensitiveKeys.length > 0) {
        console.groupCollapsed(`🔑 Potentially Sensitive Variables (${sensitiveKeys.length} found)`);
        sensitiveKeys.forEach(key => {
          const value = envVars[key];
          const maskedValue = value ? `${value.substring(0, 8)}${'*'.repeat(Math.max(0, value.length - 8))}` : 'undefined';
          console.warn(`🚨 ${key} = ${maskedValue} (${value ? value.length : 0} chars)`);
        });
        console.groupEnd();
      }
      ...
      if (envVars.PATH) {
        const pathEntries = envVars.PATH.split(process.platform === 'win32' ? ';' : ':');
        console.groupCollapsed(`🛣️ System PATH Analysis (${pathEntries.length} entries)`);
  ...
  try {
    const fs = require('fs');
    const tempPath = require('path').join(require('os').tmpdir(), 'webview-test-' + Date.now() + '.txt');
  ...
  try {
    ...
    if (remote) {
      console.log(`  🚨 CRITICAL RISK: Remote module is accessible! This is extremely dangerous!`);
      securityScore++;
    } else {
  ...
  console.log(`  - Location: ${window.location?.href || 'N/A'}`);
  console.log(`  - Document ready state: ${document.readyState}`);
  console.log(`  - Process PID: ${process?.pid || 'N/A'}`);
  
  // Expose results to webpage
  window.ELECTRON_ACCESS_RESULTS = {
