...
/**
 * Desktop Release perf runner (daemon):
 * - Run desktop release perf jobs on a fixed interval until user terminates this process.
 *
...
function spawnJob({ repoRoot, jobArgs }) {
  const child = spawn(
    process.execPath,
    ['development/perf-ci/run-desktop-perf-release.js', ...jobArgs],
    {
      cwd: repoRoot,
      stdio: 'inherit',
      env: {
...
async function main() {
  ...
  while (!stopping) {
    ...
    // eslint-disable-next-line no-console
    console.log(`[perf] desktop release job start: ${startedAt}`);
