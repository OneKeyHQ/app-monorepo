 ...
 *
 * Notes:
 * - performance-server is managed by the underlying job runner (`run-ios-perf-detox.js`):
...
function spawnJob({ repoRoot, jobArgs }) {
  const child = spawn(
    process.execPath,
    ['development/perf-ci/run-ios-perf-detox-release.js', ...jobArgs],
    {
      cwd: repoRoot,
      stdio: 'inherit',
      env: {
...
async function main() {
  ...
  while (!stopping) {
    ...
    // eslint-disable-next-line no-console
    console.log(`[perf] release job start: ${startedAt}`);
    // Keep a handle so SIGINT/SIGTERM can stop an in-flight job.
    // perf-server will remain running (managed by the job runner).
