...
if [ -n "$NATIVE_PATCHES" ]; then
  ...
  while IFS= read -r patch_file; do
    ...
    if [ -d "$pkg_dir" ]; then
      ...
      if echo "$pkg_name" | grep -qiE '(react-native|expo[-+]|^expo$|@expo/|@react-native|@onekeyfe.*react-native|hermes)'; then
        is_native=true
      fi
    fi
...
if [ -n "$YARN_LOCK_CHANGED" ]; then
  ...
  NATIVE_PKG_PATTERNS='react-native[-@]|@react-native/|@react-native-|@react-native-community/|@onekeyfe/react-native-|@sentry/react-native|/react-native-|expo[-@]|@react-navigation/|lottie-react-native|hermes-engine'
