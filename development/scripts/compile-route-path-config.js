...
class RouteCompiler {
  ...
  extensionOrder() {
    const platformExtensions = {
      web: ['.web', '.web-only'],
      'web-embed': ['.web-embed', '.web-only', '.web'],
      ext: ['.ext', '.web'],
      desktop: ['.desktop', '.web'],
      ios: ['.ios', '.native'],
  ...
  platformValue(property, node) {
    ...
    const values = {
      ...
      isWebEmbed: this.target === 'web-embed',
  ...
  compileRoot() {
    ...
    const childEntries = this.resolveExport(
      rootRouterFile,
      'rootRouterPathConfigSources',
    );
    if (!Array.isArray(childEntries)) {
      throw new Error('rootRouterPathConfigSources must resolve to an array');
    }
    for (const childEntry of childEntries) {
      const root = roots.find((route) => route.name === childEntry.name);
      if (!root) {
        throw new Error(`Missing root route ${childEntry.name}`);
      }
      if (
        typeof childEntry.pathConfigFile !== 'string' ||
        typeof childEntry.pathConfigExport !== 'string'
      ) {
        throw new Error(
          `Invalid path config source for root route ${childEntry.name}`,
        );
      }
      const childFile = this.resolveModule(
        rootRouterFile,
        childEntry.pathConfigFile,
      );
      if (!childFile) {
        throw new Error(
          `Cannot resolve ${childEntry.pathConfigFile} for ${childEntry.name}`,
        );
      }
      const children = this.resolveExport(
        childFile,
        childEntry.pathConfigExport,
      );
      if (!Array.isArray(children)) {
        throw new Error(
          `${childEntry.pathConfigExport} must resolve to an array`,
        );
      }
...
const compileTargetMode = (target, mode) => {
  ...
  for (const target of selectedTargets) {
    for (const mode of modes) {
      files.set(
        ...
        stableJson(compileTargetMode(target, mode)),
      );
    }
    files.set(
  ...
  if (!check) {
    for (const target of selectedTargets) {
      const legacyFile = path.join(
        outputDirectory,
        `routePathConfig.generated.${target}.json`,
      );
